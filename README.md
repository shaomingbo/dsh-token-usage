...
- **Honest cost** — estimates use an embedded versioned price snapshot (LiteLLM-derived) plus your aliases, custom prices, and provider multipliers. Original valuations are immutable; a current-rule view revalues when you change rules. Unknown prices are excluded and reported as coverage, never guessed.
