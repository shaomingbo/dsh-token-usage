...
// Adaptive official-observation cadence, CodexBar-style: refresh fast while
// the user is actually looking (the sidebar entry polls `entry-summary` every
// 60s — a previous poll within the stream window counts as attention), slower
// for isolated calls right after a reopen lull, and never at all when no
// polls arrive (GUI closed). Caps stay far above anything that could hammer
// the providers' read-only usage endpoints.
...
/**
 * Pure decision for the piggy-backed observation refresh: `pollerActive` marks
 * a steady client poll stream, `minIntervalMs` is the tier it maps to, and
 * `due` says whether a refresh may start now.
 */
...
export function apply(ctx, config = {}) {
  ...
  const capabilities = createProviderCapabilities({
    chatgptGrok: { credentials: ctx.credentials, settings: ctx.settings, logger: capabilityLogger, fetchImpl },
    antigravity: { credentials: ctx.credentials, settings: ctx.settings, logger: capabilityLogger, fetchImpl },
  })
  ...
  // Custom model facts (e.g. host-configured gpt-6-astra) come from the public
  // configured PiAiModelProfile fields plus, when present, the host-resolved
  // LlmResolvedModelInfo as a cross-check; only whitelisted fields cross this
  // owner seam — never URLs, headers, credentials or fetch implementations.
  let llmModelInfo
  ...
  // Trusted custom-model facts from public configured profile fields plus the
  // host-resolved model info as a cross-check; fixed gap vocabulary, whitelisted
  // fields only. See lib/capabilities/codex-native/model-facts.js.
  const codexModelFacts = createCodexModelFacts({
    getSettings: ns => ctx.settings.get(ns),
    // Dynamic: the llm service may appear after this plugin loads.
  ...
  // Shared observation single-flight: background cadence refreshes and manual
  // `refresh-observations` RPCs join the same in-flight round instead of
  // stacking duplicate provider hits. `…IncludesManualCookie` records whether
  // the in-flight round already observes Ollama Cloud with the manual-cookie
  // opt-in. Instance-scoped, so a plugin instance can never join another
  // instance's round.
  let observationRefreshPromise = null
  let observationRefreshIncludesManualCookie = false
  ...
  /** Merge the caller signal with a hard round deadline so nothing can outlive the round. */
  function roundSignalWithDeadline(signal) {
  ...
  async function performObservationRefresh(body = {}, signal) {
    const deadline = AbortSignal.timeout(observationRoundTimeoutMs())
    ...
    const [subscriptions, antigravity, glmCredential, ollamaKey] = await Promise.all([
      bounded(capabilities.chatgptGrok.handleRpc('usage', { refresh: true }, roundSignal)),
      bounded(capabilities.antigravity.handleRpc('usage-all', { refresh: true }, roundSignal)),
      bounded(resolveGlmCredential(), null),
      bounded(ctx.credentials.resolve('OLLAMA_API_KEY').catch(() => undefined), undefined),
    ])
    ...
    // Adapter observations run concurrently — each lane owns its own failures
    // — and are appended in a fixed order (local, glm, cloud api-key, cloud
    // manual cookie) after every lane settles, so no lane waits on the others
    // while the returned `adapters` array stays deterministic.
    const glmLane = glmCredential == null ? null : (async () => {
      try {
    ...
    const lanes = [
      ...
      typeof ollamaKey?.value === 'string' && ollamaKey.value
        ? bounded(observeAdapter('ollama-cloud', roundSignal, { kind: 'api_key', apiKey: ollamaKey.value }), timeoutEntry('ollama-cloud'))
        : null,
      body.ollamaManualCookie === true ? bounded(observeManualCookie(roundSignal), timeoutEntry('ollama-cloud')) : null,
    ]
    const adapterObservations = (await Promise.all(lanes.map(lane => lane ?? null))).filter(entry => entry !== null)

    // Capability calls that did not settle inside the round are reported as
    // missing providers instead of silently disappearing from the payload.
    const missing = []
    if (subscriptions === undefined) missing.push(timeoutEntry('chatgpt-grok'))
    if (antigravity === undefined) missing.push(timeoutEntry('antigravity'))
    return {
  ...
  /**
   * A manual refresh carrying `ollamaManualCookie` may join an in-flight round
   * that started without the cookie opt-in (e.g. a background cadence round).
   * The shared round is never restarted; once it settles, exactly the missing
   * item — the manual-cookie Ollama Cloud observation — runs once with a fresh
   * bounded signal and the result is merged into the returned payload.
   */
  function observeManualCookieCatchUp(result, signal) {
    return observeManualCookie(roundSignalWithDeadline(signal)).then(entry => ({
      ...result,
      adapters: [...result.adapters, entry],
    }))
  }

  /**
   * Shared single-flight entry: background cadence refreshes and manual
   * `refresh-observations` RPCs both come through here. When a round is
   * already in flight, callers join it (same promise) instead of stacking a
   * second round on the providers. Every round — manual or background —
   * updates the cadence watermark when it settles.
   */
  function runObservationRefresh(body = {}, signal) {
    const manualCookie = body.ollamaManualCookie === true
    if (observationRefreshPromise !== null) {
      if (!manualCookie || observationRefreshIncludesManualCookie) return observationRefreshPromise
      return observationRefreshPromise.then(result => observeManualCookieCatchUp(result, signal))
    }
    observationRefreshIncludesManualCookie = manualCookie
    const promise = performObservationRefresh(body, signal).finally(() => {
      lastObservationRefreshAt = Date.now()
      observationRefreshIncludesManualCookie = false
      if (observationRefreshPromise === promise) observationRefreshPromise = null
    })
    observationRefreshPromise = promise
    return promise
  }

  /** Fire-and-forget, at most once per adaptive interval; failures stay silent. */
  function maybeRefreshObservations(pollerActive = false) {
    if (!observationRefreshDue({ lastRefreshAt: lastObservationRefreshAt, pollerActive }).due) return
    void runObservationRefresh({}).catch(() => {})
  }
  ...
  async function collectConnections(signal) {
    ...
    return {
      ...
      { providerId: 'glm', displayName: 'GLM / Z.AI', configured: glmConfigured, credentialKind: 'raw_authorization', credentialRef: glmCredential?.ref ?? 'ZAI_CODING_CN_API_KEY', observationSource: 'official_plugin_internal_api' },
  ...
  async function handleAccountRequest(endpoint, payload, signal) {
    try {
      ...
      if (endpoint === 'summary' || endpoint === 'connections') {
        ...
        ensureConnectionAccounts(collected.connections)
        maybeRefreshObservations(noteClientPoll())
      ...
      if (endpoint === 'observe-provider') {
        ...
        const connection = { id: typeof body.connectionId === 'string' ? body.connectionId : `${providerId}:default` }
        let credential = {}
        if (providerId === 'glm') {
          const glmCredential = await resolveGlmCredential()
          credential = { rawAuthorization: glmCredential?.value }
          const observation = await glmAdapterFor(glmCredential?.origin ?? 'https://api.z.ai').observe({
      ...
      if (endpoint === 'refresh-observations') {
        if (body.refresh !== true) throw Object.assign(new Error('provider observation refresh requires explicit refresh=true'), { code: 'explicit-refresh-required' })
        // Manual refresh bypasses the cadence gate but still joins an
        // in-flight round instead of stacking a duplicate provider hit.
        return envelopeSuccess(await runObservationRefresh(body, signal))
      }
  ...
  ctx.connection.rpc.handle(CHANNEL, async (endpoint, payload, signal) => {
    try {
      ...
      switch (endpoint) {
        ...
        case 'entry-summary':
          // The sidebar entry polls every 60s; piggy-back an adaptively
          // throttled official observation refresh so entry meters stay
