...
import { nanoToUsdString } from './ledger/pricing.js'
...
export function apply(ctx) {
  ...
  async function importSnapshot(snap) {
    ...
    const header = { ...inspection.header, revision: snap.revision }
  ...
  // historical import owns everything before the watermark.
  try {
  ...
  ctx.connection.rpc.handle(CHANNEL, async (endpoint, payload, signal) => {
    try {
      ...
      switch (endpoint) {
        ...
        case 'restore':
          led.restoreFrom(String(body.path), { mode: body.mode === 'replace' ? 'replace' : 'merge' })
          return envelopeSuccess({})
        case 'purge': {
          const days = Number(body.days)
          if (!Number.isFinite(days) || days <= 0) throw new Error('purge requires a positive day count')
          const cutoff = Date.now() - days * DAY
          led.purgeBefore(cutoff)
          return envelopeSuccess({ cutoff })
        }
        default:
