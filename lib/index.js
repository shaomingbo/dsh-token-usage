...
const ACCOUNT_USAGE_ENDPOINTS = Object.freeze([
  ...
  'refresh-observations', 'connection-action',
])
...
export function apply(ctx, config = {}) {
  ...
  // 0.1.2 mounts generic channels. 0.1.5 Connection.handle() fails closed
  // without webServer on the Connection fiber; /api Fetch routes are the
  // 0.1.5 browser transport (same pattern as dsh-subscription-search).
  ...
  registerRpc(ANTIGRAVITY_CHANNEL, antigravityRpc, { authority: 'loopback' })
  ...
  registerRpc(CHANNEL, handleTokenRequest, { authority: 'loopback' })
