...
import { resolveDataDir as resolveProfileDataDir, resolveDataIdentity } from './observations/data-dir.js'
import {
...
// `webServer` is declared for the 0.1.5-rc.1 contract: registering an RPC
// channel through `connection.rpc.handle` mounts its HTTP route as an effect on
// THIS fiber's `webServer` there (0.1.2 mixins made the access implicit; the
// 0.1.5 loader requires the explicit declaration, as the 015-adapted
// subscription-search plugin already does).
export const inject = ['connection', 'credentials', 'sessionPersistence', 'settings', 'timer', 'webServer']
...
export function apply(ctx, config = {}) {
  ...
  // 0.1.5 Connection registers RPC onto the caller fiber's webServer. The
  // loaderFixes-verified form: register from inside a runtime webServer inject
  // scope, so connection is read from a context whose fiber can resolve it.
  function registerRpc(channel, handler, options) {
    ctx.inject(['webServer'], (rpcCtx) => {
      rpcCtx.connection.rpc.handle(channel, handler, options)
    })
  }
  ...
  registerRpc('/subscription-antigravity', (endpoint, payload, signal) => (
  ...
  registerRpc(ACCOUNT_CHANNEL, handleAccountRequest, { authority: 'loopback' })

  registerRpc(CHANNEL, async (endpoint, payload, signal) => {
    try {
      const led = requireLedger()
      const body = payload ?? {}
      switch (endpoint) {
