...
export function apply(ctx, config = {}) {
   ...
   * accounts stay archived; only the active Antigravity account routes local
   * traffic.
   */
  function ensureConnectionAccounts(connections) {
    ...
    const activeAntigravity = lastAntigravityState?.activeAccountId ?? null
    for (const connection of connections) {
      if (connection.configured !== true) continue
      if (connection.providerId === 'glm' && led.listAccounts().some((account) => !account.archived && account.providerId === 'glm')) continue
    ...
    try {
      ...
      if (endpoint === 'summary' || endpoint === 'connections') {
        ...
        return envelopeSuccess({
          product: { name: 'DSH Accounts & Usage', version: '5.0.17' },
