...
export function apply(ctx, config = {}) {
  ...
  void capabilitiesReady.then(() => collectConnections()).then((connections) => {
    try {
      ensureConnectionAccounts(connections)
    } catch (error) {
  ...
  async function refreshAllObservations(body = {}, signal) {
    ...
    if (glmCredential) {
      try {
        ...
        adapterObservations.push({ providerId: 'glm', observation })
      } catch (error) {
        adapterObservations.push({ providerId: 'glm', error: { code: error?.code ?? 'provider-failed', message: 'provider observation failed' } })
      }
    }
  ...
  /** Collect the current provider-connection set from all capabilities. */
  ...
  async function handleAccountRequest(endpoint, payload, signal) {
    try {
      ...
      if (endpoint === 'summary' || endpoint === 'connections') {
        const [connections, modelCatalogConfigured] = await Promise.all([
          collectConnections(signal),
          credentialConfigured('OLLAMA_API_KEY'),
        ])
        ensureConnectionAccounts(connections)
        ...
        return envelopeSuccess({
          ...
          connections,
          modelCatalogs: [{ ...ollamaModels.status(), credentialConfigured: modelCatalogConfigured }],
