...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let credentialRequestSeq = 0
    function credentialRequest(payload) {
      credentialRequestSeq += 1
      return { rpcId: `tu-credential-${Date.now()}-${credentialRequestSeq}`, payload }
    }

    async function setCredential(ctx, ref, value) {
      const response = await ctx.connection.api.credentials.set(credentialRequest({ ref, value }))
      ...
      const describe = await ctx.connection.api.credentials.describe(credentialRequest({ refs: [ref] }))
