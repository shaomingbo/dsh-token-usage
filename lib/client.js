...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function setCredential(ctx, ref, value) {
      let response
      try {
        response = await ctx.connection.api.credentials.set({ ref, value })
      } catch (error) {
        // The credentials.set value schema is an empty object; some transports
        // wrap or validate the reply — an explicit ok:false still means
        // failure, anything else is verified through describe below.
        const result = error?.message?.startsWith('[{')
          ? null
          : (error?.result ?? null)
        if (result?.ok === false) throw new Error(result.error?.message ?? 'credential save failed')
      }
      const describe = await ctx.connection.api.credentials.describe({ refs: [ref] }).catch(() => null)
      const configured = describe?.credentials?.[ref]?.configured === true
        || describe?.value?.credentials?.[ref]?.configured === true
        || describe?.result?.credentials?.[ref]?.configured === true
      if (!configured) throw new Error('凭据保存未生效（写入后 describe 未确认）')
    }
