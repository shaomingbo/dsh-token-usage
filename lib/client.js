...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AccountInsight({ ctx, store, state, t, compact = false }) {
      ...
      const detail = useAsync(() => rpc(ctx, 'inspect', { kind: 'pool', id, filter: { time: { preset: '30d' } }, now: Date.now() }), [id])
      ...
      const refreshOfficial = () => {
        ...
        void accountRpc(ctx, 'refresh-observations', { refresh: true, ollamaManualCookie: withCookie })
          .then((payload) => {
            ...
            // the fresh observation is visible the moment you go back.
    ...
    function useDashboard(ctx, state, t) {
      ...
      const reload = React.useCallback(() => {
        ...
        void rpc(ctx, 'query', {
          // now: Date.now() keeps every poll computing fresh — cached official
          // windows would otherwise trail a just-run observation refresh.
          now: Date.now(),
