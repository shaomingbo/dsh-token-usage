...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const v3Css = `
      ...
      .tu3-dock-acc { padding: 13px 16px 11px; border-bottom: 1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.04)); }
        ...
        state.account === null || state.account === 'unassigned'
          ? h('section', { className: 'tu3-card', style: { padding: '16px 18px' } },
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 12 } },
                orderedPools.map((pool) => h('div', { key: pool.id, style: { border: `1px solid ${C.borderFaint}`, borderRadius: 12, padding: '13px 15px' } },
    ...
    function DockPanel({ ctx, store, state, t }) {
      ...
      const body = () => {
        ...
        return h('div', { style: { paddingBottom: 18 } },
          ...
          h('div', { style: { padding: '16px 0 4px' } },
            h('div', { className: 'tu3-k', style: { margin: '0 16px 8px' } }, t('poolsMonthly')),
            orderedPools.map((pool) => h('div', { key: pool.id, className: 'tu3-dock-acc' },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.textSecondary, marginBottom: 6 } },
          ...
          tightest ? h('div', { style: { margin: '10px 16px 0', padding: '9px 12px', borderRadius: 9, background: C.bgMuted, color: C.textSecondary, fontSize: 11.5 } }, paceNote(tightest, t)) : null,
          h('div', { style: { margin: '14px 16px 0', borderTop: `1px solid ${C.borderFaint}`, paddingTop: 12 } },
