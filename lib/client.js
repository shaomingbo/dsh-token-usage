...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** One official window row: label, percent bar, reset countdown, optional watch pin. */
    function OfficialWindowRow({ win, color, t, watched = false, onToggleWatch = null }) {
      ...
      return h('div', { className: 'tu3-winrow' },
        h('div', { className: 'tu3-winlab' },
          h('span', { title: win.label ?? win.externalKey ?? '' }, win.label ?? win.externalKey ?? ''),
          h('span', null,
            h('b', null, fmtPct(used)),
            win.resetsAt ? h('span', { style: { marginLeft: 8 } }, `${t('resetsInLabel')} ${countdown(win.resetsAt - Date.now())}`) : null,
    ...
    function DuoMeter({ pool, index = 0, t, compact = false, primaryWin = null }) {
      ...
      if (officialWindows.length > 0) {
        if (compact) {
          ...
          const used = Math.max(0, Math.min(100, primary.percentUsed ?? 0))
          const risk = used >= 90
          ...
          const primaryLabel = `${watched ? '★ ' : ''}${primary.label ?? primary.externalKey ?? ''}${source ? ` · ${source}` : ''}`
          return h('div', null,
            h('div', { className: 'tu3-winlab' },
              h('span', { title: primaryLabel }, primaryLabel),
              h('span', null, primary.resetsAt ? `${t('resetsInLabel')} ${countdown(primary.resetsAt - Date.now())}` : null),
            ),
            h('div', { className: 'tu3-duo-track', style: risk ? { background: C.errorBg } : undefined },
              h('i', { className: 'tu3-duo-fill', style: { width: `${used}%`, background: risk ? C.error : color } }),
            ),
        ...
        officialWindows.length > 0
          ? h('div', null,
              ...
              observedTime ? h('p', { style: { ...mutedStyle, marginTop: 8 } },
                ...
                account.official.sourceKind === 'official_ui' ? ` · ${t('sourceOfficialUi')}` : '',
      ...
      const body = () => {
        ...
        return h('div', { style: { paddingBottom: 18 } },
          ...
          h('div', { style: { marginTop: `${S.l}px`, borderTop: `1px solid ${C.borderFaint}`, paddingTop: 6 } },
            ...
            const dockRowPct = dockRowWin?.percentUsed ?? pool.usedPct
            ...
            return h('div', {
              ...
              h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: FONT.body, color: C.textSecondary, marginBottom: 6 } },
                ...
                h('span', { style: { display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 } },
                  ...
                  h('b', { style: { color: C.text, font: `650 ${FONT.body}px ui-monospace, monospace` } }, dockRowPct == null ? '—' : fmtPct(dockRowPct)),
                ),
              ),
    ...
    function SidebarEntry(props) {
      ...
      const resetBit = focus?.resetsAt ? ` · ${t('resetIn')} ${countdown(focus.resetsAt - Date.now())}` : ''
      const caption = focus == null
        ? t('entryUnconfigured')
        : usedPct == null
          ? `${pinBit}${focus.name} · ${t('entryNoQuota')}`
          : `${pinBit}${focus.name}${windowBit}${resetBit}`
      ...
      const tightestBit = pinned && summary?.tightest != null && summary.tightest.usedPct != null
        ...
        : ''
      const aria = [title, pctLabel, caption, monthBit, tightestBit].filter(Boolean).join(' ')
