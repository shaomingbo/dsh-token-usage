...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Half-ring level meter: one arc filled by the watched quota level.
    ...
    function MeterGlyph({ usedPct = null, risk = false }) {
      const arc = Math.PI * 5.25
      const pct = Math.max(0, Math.min(100, Number.isFinite(usedPct) ? usedPct : 0))
      const fill = usedPct != null && pct > 0.5
        ? h('path', {
            d: 'M 2.75 12.5 A 5.25 5.25 0 0 1 13.25 12.5', fill: 'none',
            stroke: risk ? 'var(--dsw-alias-state-error-primary, #ec1313)' : 'currentColor',
            strokeWidth: 2, strokeLinecap: 'round', strokeDasharray: `${(arc * pct) / 100} ${arc}`,
          })
        : null
      return h('svg', { width: 16, height: 16, viewBox: '0 0 16 16', 'aria-hidden': true, style: { flexShrink: 0, marginTop: 2 } },
        h('path', { d: 'M 2.75 12.5 A 5.25 5.25 0 0 1 13.25 12.5', fill: 'none', stroke: 'currentColor', strokeOpacity: 0.25, strokeWidth: 2, strokeLinecap: 'round' }),
        fill,
      )
    }
    ...
    function SidebarEntry(props) {
      ...
      if (styleKind === 'badge') {
        return h('button', { type: 'button', style: footerButtonStyle(), title: aria, onClick: open },
          ...
          h('span', { style: { minWidth: 0, flex: 1 } },
            ...
            h('span', { style: { display: 'block', fontSize: 10, color: C.textSecondary, marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, caption),
          ),
        )
      }
      ...
      return h('button', { type: 'button', style: footerButtonStyle(), title: aria, onClick: open },
        ...
        h('span', { style: { minWidth: 0, flex: 1 } },
          ...
          h('span', { style: { display: 'block', fontSize: 10, color: C.textSecondary, marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, caption),
        ),
      )
    }
