...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      ollamaStep3: 'Paste the session value below (a whole Cookie header works too), tick the consent box and save. Marked official page (brittle).',
    ...
    const zh = {
      ...
      ollamaStep3: '在下方粘贴 session 的值（整段 Cookie Header 也可以），勾选同意后保存。观察标记为官方页面（脆弱）。',
    ...
    function sanitizeOllamaCookieHeader(header) {
      ...
      // A bare value pasted straight from the DevTools cookie table: treat it
      // as the `session` value instead of rejecting it.
      const maybeRecognized = OLLAMA_COOKIE_NAMES.has(input.split('=')[0].trim())
      if (!input.includes(';') && !maybeRecognized) return `session=${input}`
      const selected = []
      const seen = new Set()
      const unrecognized = []
      ...
      if (selected.length === 0) {
        throw new Error(`未识别到白名单内的 Ollama 会话 Cookie(允许: session / ollama_session / next-auth…);${unrecognized.length ? `检测到的名称: ${unrecognized.join(', ')}` : '输入不包含 name=value 对——直接粘贴 session 的值即可'}`)
      }
