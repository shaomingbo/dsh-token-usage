...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      settingsManageConnection: 'Manage',
      settingsCollapseConnection: 'Collapse',
      settingsManageHint: 'Managing a connection opens the same connection controls used by the dashboard insight.',
      settingsModelsTitle: 'Model catalogs (cached facts)',
      settingsModelsUnknown: 'Model list unknown for this provider — use the explicit sync action in the connection controls to refresh it.',
      settingsModelsCount: '{count} model(s) cached',
      settingsAntigravityActive: 'Active Antigravity account: {id}',
      settingsAntigravityNone: 'No active Antigravity account.',
    }
    ...
    const zh = {
      ...
      settingsManageConnection: '管理',
      settingsCollapseConnection: '收起',
      settingsManageHint: '管理连接会打开与洞察页相同的连接控件。',
      settingsModelsTitle: '模型目录（缓存事实）',
      settingsModelsUnknown: '该提供方的模型列表未知——可在连接控件中显式同步刷新。',
      settingsModelsCount: '已缓存 {count} 个模型',
      settingsAntigravityActive: 'Antigravity 活动账户：{id}',
      settingsAntigravityNone: '当前没有活动的 Antigravity 账户。',
    }
    ...
    async function setCredential(t, ctx, ref, value) {
      // ctx.connection.api is the high-level Web facade: it accepts the bare
      // method payload and constructs RpcRequest internally (same contract as
      // DSH's ProviderEditor / CustomProviderCard).
      const response = await ctx.connection.api.credentials.set({ ref, value })
      if (response?.result?.ok !== true) {
        throw new Error(response?.result?.error?.message ?? t('credentialSaveFailed'))
      }
      // Authoritative value-free confirmation through the same public API.
      const describe = await ctx.connection.api.credentials.describe({ refs: [ref] })
      if (describe?.result?.ok !== true) {
        throw new Error(describe?.result?.error?.message ?? t('credentialDescribeFailed'))
      }
      if (describe.result.value.credentials?.[ref]?.configured !== true) {
    ...
    function ConnectionSection({ ctx, store, t, identity, onChanged, facts }) {
      ...
      const loginActionRef = React.useRef(0)
      ...
      const waitForLogin = async (providerId, loginId, expiresAt) => {
        ...
        stopLocalLoginWait()
        // Follow the provider's advertised deadline, not a three-minute UI
        // budget. Providers without one still have a bounded local wait.
        const hardDeadline = Date.now() + 30 * 60 * 1000
        const deadlineFor = value => Number.isFinite(value) && value > 0 ? Math.min(value, hardDeadline) : hardDeadline
        const wait = { loginId, live: true, deadline: deadlineFor(expiresAt) }
        ...
        try {
          for (let attempt = 0; attempt < 1200; attempt += 1) {
            ...
            if (status.kind === 'pending' && status.challenge) {
              if (Number.isFinite(status.challenge.expiresAt)) wait.deadline = deadlineFor(status.challenge.expiresAt)
              setLogin({ provider: providerId, ...status.challenge })
              setConnecting(false)
            }
            if (status.kind === 'succeeded') { setLogin(null); setConnecting(false); refreshData(); onChanged?.(); return }
            if (status.kind === 'failed' || status.kind === 'cancelled') {
              // The live wait/identity guards above also protect reattached
              // panels. A terminal Host result ends their local card, without
              // issuing a second cancel/logout or leaving Connect disabled.
              setLogin(null)
              setConnecting(false)
              throw new Error(status.message ?? t('authorizationStatus', { status: status.kind }))
            }
      ...
      const restorePendingLogin = async (providerId, action) => {
        const actionLive = () => localLifecycleLive() && action === loginActionRef.current
        setRestoring(true)
        try {
          const pending = await accountRpc(t, ctx, 'connection-action', { provider: providerId, action: 'pending-login', params: { provider: providerId } })
          if (!actionLive()) return
          const found = pending?.login ?? null
          setLogin(found?.loginId ? { provider: providerId, ...found } : null)
          setConnecting(!found?.loginId && pending?.active === true)
          if (found?.loginId) void waitForLogin(providerId, found.loginId, found.expiresAt).catch(error => { if (actionLive()) setMessage(error.message) })
        } finally {
          if (actionLive()) setRestoring(false)
        }
      }
      ...
      React.useEffect(() => {
        ...
        const action = loginActionRef.current
        void restorePendingLogin(providerId, action)
          .catch(error => { if (localLifecycleLive() && action === loginActionRef.current) setMessage(error.message) })
      ...
      const connect = async () => {
        ...
        try {
          ...
          await waitForLogin(provider, challenge.loginId, challenge.expiresAt)
        } catch (error) {
          // A late failure (for example an in-flight poll rejection landing
          // after unmount/cancel) must not touch dead or superseded state either.
          if (!actionLive()) return
          setMessage(error.message)
          setLogin(null)
          setConnecting(false)
        } finally {
          if (actionLive()) setBusy(null)
        }
      }
      const cancelLogin = async () => {
        ...
        try {
          ...
          if (actionLive()) { setLogin(null); setConnecting(false); setRestoring(false) }
        } catch (error) {
          if (actionLive()) {
            setMessage(error.message)
            // Cancellation failure does not invalidate the host operation.
            // Recover its id even if the original start reply is still late.
            if (isSubscriptionProvider(provider)) {
              try { await restorePendingLogin(provider, action) }
              catch (recoveryError) { if (actionLive()) setMessage(`${error.message}; ${recoveryError.message}`) }
            } else if (login?.loginId) {
              void waitForLogin(provider, login.loginId, login.expiresAt).catch(error => { if (actionLive()) setMessage(error.message) })
            }
          }
        } finally {
          if (actionLive()) setBusy(null)
      ...
      return h('div', null,
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: S.m } },
          ...
          restoring ? h('span', { style: mutedStyle }, t('loading')) : null,
          h('span', { style: { marginLeft: 'auto', display: 'flex', gap: 6 } },
            ...
            oauthProviders.has(provider) ? h('button', {
              type: 'button', style: compactButtonStyle, disabled: busy !== null || restoring || (!connection.configured && (connecting || login !== null)),
        ...
        (login || connecting) ? h('div', { style: { ...cardStyle, marginTop: S.m } },
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
            ...
            h('button', { type: 'button', style: { ...compactButtonStyle, marginLeft: 'auto' }, disabled: busy === `cancel-login:${provider}`, onClick: cancelLogin }, t('cancelLogin'))),
          login?.userCode ? [
    ...
    // ---------- Accounts & Models native settings section (P1-C) ----------
    ...
    // never triggers a quota refresh, model sync or login by itself. Even the
    // reused ConnectionSection runs in facts mode here (C-001): expanding a
    // row hands it the already-loaded facts object, and its explicit actions
    // reload the same object — no `summary` call, no account bootstrap, no
    // observation start, on expand, switch, refresh or after any action.
    // The account insight keeps its own summary-based strategy.
    function AccountsModelsSection({ ctx, store, t }) {
      ...
      const modelCatalogs = facts.status === 'ready' ? (facts.value.modelCatalogs ?? []) : []
      const antigravity = facts.status === 'ready' ? facts.value.antigravity : null

      const catalogRow = (providerId) => modelCatalogs.find(item => item.providerId === providerId) ?? null

      const manageIdentity = (connection) => ({
        providerId: connection.providerId,
        connectionId: connection.connectionId ?? `${connection.providerId}:default`,
      })

      const connectionRowElement = (connection) => {
        const key = connectionKey(connection)
        const open = expanded === key
        const catalog = catalogRow(connection.providerId)
        return h('div', { key, style: { ...cardStyle, padding: '10px 12px' } },
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' } },
            h('span', { style: { fontSize: 13 } },
              h('b', null, connection.displayName ?? connection.providerId),
              h('span', { className: 'tu3-tag', style: { ...sourceBadgeStyle(connectionVerified(connection)), marginLeft: 8 } },
                connectionStateText(connection, t))),
            h('code', { style: { ...badgeStyle, fontSize: 10.5 } }, `${connection.providerId} · ${connection.connectionId ?? `${connection.providerId}:default`}`),
            connection.credentialRef ? h('code', { style: badgeStyle }, connection.credentialRef) : null,
            h('span', { style: { marginLeft: 'auto' } },
              h('button', {
                type: 'button', style: compactButtonStyle,
                onClick: () => setExpanded(open ? null : key),
              }, open ? t('settingsCollapseConnection') : t('settingsManageConnection')))),
          open ? [
            h('p', { key: 'hint', style: { ...mutedStyle, margin: '8px 0 0', fontSize: 11 } }, t('settingsManageHint')),
            h('div', { key: 'manage', style: { marginTop: 8 } },
              // Facts mode: the panel reads and reloads this section's shared
              // read-only connections payload; onChanged stays unused here so
              // each explicit action reloads the facts exactly once.
              h(ConnectionSection, { ctx, store, t, identity: manageIdentity(connection), facts })),
          ] : null,
        )
      }

      const catalogElement = (catalog) => {
        const known = typeof catalog?.modelCount === 'number' && catalog.modelCount > 0
        return h('div', { key: catalog?.providerId ?? 'unknown', style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', padding: '6px 0' } },
          h('code', { style: badgeStyle }, catalog?.providerId ?? 'unknown'),
          known
            ? h('span', { style: mutedStyle }, t('settingsModelsCount', { count: catalog.modelCount }))
            : h('span', { style: { ...mutedStyle, color: C.warn } }, t('settingsModelsUnknown')),
        )
      }
      ...
      if (facts.status !== 'ready') {
        ...
        body = h('div', { style: { display: 'grid', gap: S.s } },
          connections.map(connectionRowElement),
          h('div', { key: 'models', style: { ...cardStyle, padding: '10px 12px' } },
            h('div', { style: { fontWeight: 650, fontSize: 12.5, marginBottom: 2 } }, t('settingsModelsTitle')),
            modelCatalogs.length > 0 ? modelCatalogs.map(catalogElement) : h('span', { style: { ...mutedStyle, color: C.warn } }, t('settingsModelsUnknown')),
            h('p', { style: { ...mutedStyle, margin: '6px 0 0', fontSize: 11 } },
              antigravity?.activeAccountId
                ? t('settingsAntigravityActive', { id: antigravity.activeAccountId })
                : t('settingsAntigravityNone'))),
        )
      }
      ...
      return h('div', { style: { marginBottom: S.l } },
        ...
        facts.status === 'ready'
          ? h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: S.s } },
              h('span', { style: mutedStyle }, t('settingsAccountsCount', { count: connections.length })),
              h('span', { style: { marginLeft: 'auto' } },
                h('button', {
    ...
    function SidebarEntry(props) {
      ...
      // global tightest window. A missing level renders as '—', never 0%;
      // an expired window reads as '—' too, with the expired caption.
      const watchId = readSidebarWatch()
      const watchedPool = watchId != null
        ? (summary?.pools ?? []).find((pool) => pool.id === watchId) ?? null
        : null
      const pinned = watchedPool != null
      const focusSource = pinned
        ? {
            name: watchedPool.name,
            color: watchedPool.color,
            usedPct: watchedPool.window?.usedPct ?? watchedPool.usedPct ?? null,
            windowLabel: watchedPool.window?.label ?? null,
            resetsAt: watchedPool.window?.resetsAt ?? null,
            stale: watchedPool.window?.stale,
            ageMs: watchedPool.window?.ageMs ?? null,
            expired: watchedPool.window?.expired,
            brittle: watchedPool.window?.brittle,
          }
