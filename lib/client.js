  ...
  factory: (require) => {
    ...
    function ConnectionSection({ ctx, store, t, identity, onChanged }) {
      ...
      const waitForLogin = async (loginId) => {
        for (let attempt = 0; attempt < 120; attempt += 1) {
          ...
          const result = await accountRpc(t, ctx, 'connection-action', { provider, action: 'login-status', params: { loginId } })
          const status = result.status ?? result
          if (status.kind === 'succeeded') { setLogin(null); refreshSummary(); onChanged?.(); return }
      ...
      return h('div', null,
        ...
        login ? h('div', { style: { ...cardStyle, marginTop: S.m } },
    ...
    function AccountInsight({ ctx, store, state, t, compact = false }) {
      ...
      const refreshOfficial = () => {
        setRefreshing(true)
        setMessage(null)
        // For Ollama Cloud a stored cookie (saved through the explicit opt-in)
        // is what produces percentages, so the explicit refresh includes it.
        const withCookie = id.includes('ollama-cloud')
        void accountRpc(t, ctx, 'refresh-observations', { refresh: true, ollamaManualCookie: withCookie })
          .then((payload) => {
            detail.reload()
            ...
            return h('div', {
              ...
              h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: FONT.body, color: C.textSecondary, marginBottom: 6 } },
                ...
                h('span', { style: { display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 } },
                  ...
                  dockRowState.staleAge ? h('span', {
                    title: dockRowState.staleTitle, 'aria-label': dockRowState.staleTitle,
                    style: { color: C.warn, fontSize: 9, fontWeight: 500, whiteSpace: 'nowrap' },
                  }, dockRowState.staleAge) : null,
                ),
              ),
    ...
    function SidebarEntry(props) {
      ...
      const load = React.useCallback(() => (
        ...
        // completion instead of on callback return.
        rpc(t, ctx, 'entry-summary')
          .then((entrySummary) => store.update({ entrySummary }))
          .catch(() => {})
      ), [ctx, store])
      usePoll(load, 60_000)
