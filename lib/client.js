  ...
  factory: (require) => {
    ...
    function useT(locale) {
      return React.useCallback((key, params) => {
        const dict = locale?.language === 'zh' || locale?.tag === 'zh' || String(locale ?? '').startsWith('zh') ? zh : en
    ...
    // ---------- styles (theme variables, never hardcoded palette) ----------
    const overlayStyle = {
      position: 'absolute', inset: 0, zIndex: 30, pointerEvents: 'auto',
      display: 'flex', flexDirection: 'column', overflow: 'hidden',
      background: 'var(--dsw-bg-primary, #0f0f12)', color: 'var(--dsw-text-primary, #f3f4f6)',
      font: 'inherit',
    }
    const headerStyle = {
      display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px',
      borderBottom: '1px solid var(--dsw-border, #26262c)',
    }
    ...
    const tabStyle = (active) => ({
      ...
      border: 'none', background: active ? 'var(--dsw-bg-accent, #1f2937)' : 'transparent',
      color: active ? 'var(--dsw-text-primary, #f3f4f6)' : 'var(--dsw-text-secondary, #9ca3af)',
    })
    const closeStyle = {
      marginLeft: 'auto', border: 'none', background: 'transparent',
      color: 'var(--dsw-text-secondary, #9ca3af)', fontSize: 16, cursor: 'pointer', padding: '4px 10px', borderRadius: 8,
    }
    const contentStyle = { flex: 1, overflow: 'auto', padding: '20px 24px' }
    const cardStyle = {
      border: '1px solid var(--dsw-border, #26262c)', borderRadius: 12,
      padding: '14px 16px', background: 'var(--dsw-bg-secondary, #16161b)',
    }
    const cardGridStyle = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 16 }
    const metricLabelStyle = { fontSize: 12, color: 'var(--dsw-text-secondary, #9ca3af)', margin: '0 0 6px' }
    const metricValueStyle = { fontSize: 22, fontWeight: 600, margin: 0 }
    const metricSubStyle = { fontSize: 12, color: 'var(--dsw-text-secondary, #9ca3af)', margin: '4px 0 0' }
    const sectionTitleStyle = { fontSize: 14, fontWeight: 600, margin: '0 0 10px' }
    const tableStyle = { width: '100%', borderCollapse: 'collapse', fontSize: 13 }
    const thStyle = { textAlign: 'left', padding: '6px 12px 6px 0', borderBottom: '1px solid var(--dsw-border, #26262c)', fontSize: 12, color: 'var(--dsw-text-secondary, #9ca3af)', fontWeight: 500 }
    const tdStyle = { textAlign: 'left', padding: '7px 12px 7px 0', borderBottom: '1px solid var(--dsw-border, #1d1d23)' }
    const buttonStyle = {
      padding: '6px 12px', borderRadius: 8, fontSize: 13, cursor: 'pointer',
      border: '1px solid var(--dsw-border, #26262c)', background: 'var(--dsw-bg-secondary, #16161b)',
      color: 'var(--dsw-text-primary, #f3f4f6)',
    }
    const primaryButtonStyle = { ...buttonStyle, background: 'var(--dsw-bg-accent, #1d4ed8)', borderColor: 'transparent' }
    const inputStyle = {
      fontSize: 13, padding: '6px 10px', borderRadius: 8,
      border: '1px solid var(--dsw-border, #26262c)', background: 'var(--dsw-bg-primary, #0f0f12)',
      color: 'var(--dsw-text-primary, #f3f4f6)',
    }
    const selectStyle = { ...inputStyle }
    const mutedStyle = { color: 'var(--dsw-text-secondary, #9ca3af)', fontSize: 12, margin: 0 }
    const errorStyle = { color: 'var(--dsw-text-error, #f87171)', fontSize: 13 }
    const badgeStyle = {
      fontSize: 11, padding: '2px 8px', borderRadius: 999,
      background: 'var(--dsw-bg-muted, #1f1f26)', color: 'var(--dsw-text-secondary, #9ca3af)',
    }
    const avatarStyle = (size) => ({
      width: size, height: size, borderRadius: '50%', objectFit: 'cover',
      background: 'var(--dsw-bg-accent, #1f2937)', display: 'flex', alignItems: 'center', justifyContent: 'center',
      color: 'var(--dsw-text-primary, #f3f4f6)', fontWeight: 600, fontSize: size / 2.4, overflow: 'hidden',
    })
    ...
    const heatCellStyle = (level) => ({
      width: 11, height: 11, borderRadius: 3, margin: 1,
      background: ['var(--dsw-bg-muted, #1b1b22)', 'rgba(59,130,246,0.25)', 'rgba(59,130,246,0.45)', 'rgba(59,130,246,0.7)', 'rgba(59,130,246,0.95)'][level] ?? 'var(--dsw-bg-muted, #1b1b22)',
    })
    const footerButtonStyle = (wide) => ({
      display: 'flex', alignItems: 'center', gap: 8, width: wide ? 'auto' : 'auto',
      padding: '6px 10px', borderRadius: 8, cursor: 'pointer',
      border: 'none', background: 'transparent', color: 'var(--dsw-text-secondary, #9ca3af)', fontSize: 13,
    })
      ...
      return h('div', null,
        ...
        h('div', { style: cardGridStyle },
          ...
          h('div', { style: cardStyle },
            ...
            h('p', { style: metricValueStyle }, String(totals.requests)),
    ...
    function Heatmap(props) {
      ...
      const weeks = []
      for (let index = 0; index < cells.length; index += HEAT_COLUMNS) {
        weeks.push(h('div', { key: index, style: { display: 'flex', flexDirection: 'column' } }, ...cells.slice(index, index + HEAT_COLUMNS)))
      }
        ...
        h('table', { style: tableStyle },
          ...
          h('tbody', null, ...rows.map((row) =>
            h('tr', { key: `${row.sessionId}:${row.turn}:${row.step}` },
              ...
              h('td', { style: tdStyle }, fmtTokens(row.processingTokens)),
              h('td', { style: tdStyle }, row.originalUsdNano === null ? '—' : fmtUsd(row.originalUsdNano)),
    ...
    function PricingTab(props) {
      ...
      const snapshot = settings.value.priceSnapshot ?? { models: [] }
      const submit = (endpoint, payload, reset) => {
        void rpc(ctx, endpoint, payload).then(() => { reset(); settings.reload() }).catch(() => {})
      }
      return h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 } },
        h('div', { style: cardStyle },
          ...
          h('p', { style: mutedStyle }, `${snapshot.models.length} ${t('priceModels')}`),
        ),
        h('div', { style: cardStyle },
          ...
          h('div', { style: { display: 'flex', gap: 6, marginTop: 8 } },
            h('input', { style: inputStyle, placeholder: t('aliasModel'), value: aliasModel, onChange: (event) => setAliasModel(event.target.value) }),
            h('input', { style: inputStyle, placeholder: t('aliasCanonical'), value: aliasCanonical, onChange: (event) => setAliasCanonical(event.target.value) }),
            h('button', { type: 'button', style: primaryButtonStyle, onClick: () => aliasModel && aliasCanonical && submit('set-alias', { model: aliasModel, canonical: aliasCanonical }, () => { setAliasModel(''); setAliasCanonical('') }) }, t('addAlias')),
          ),
        ),
        h('div', { style: cardStyle },
          h('p', { style: sectionTitleStyle }, t('overrides')),
          h('div', { style: { display: 'flex', gap: 6, marginTop: 8 } },
            h('input', { style: inputStyle, placeholder: t('overrideModel'), value: overrideModel, onChange: (event) => setOverrideModel(event.target.value) }),
            h('input', { style: inputStyle, placeholder: t('overrideInput'), value: overrideInput, onChange: (event) => setOverrideInput(event.target.value) }),
            h('input', { style: inputStyle, placeholder: t('overrideOutput'), value: overrideOutput, onChange: (event) => setOverrideOutput(event.target.value) }),
            h('button', { type: 'button', style: primaryButtonStyle, onClick: () => overrideModel && overrideInput && overrideOutput && submit('set-override', { model: overrideModel, inputNano: Number(overrideInput), outputNano: Number(overrideOutput) }, () => { setOverrideModel(''); setOverrideInput(''); setOverrideOutput('') }) }, t('addOverride')),
          ),
        ),
        h('div', { style: cardStyle },
          h('p', { style: sectionTitleStyle }, t('multipliers')),
          h('div', { style: { display: 'flex', gap: 6, marginTop: 8 } },
            h('input', { style: inputStyle, placeholder: t('multiplierProvider'), value: multiplierProvider, onChange: (event) => setMultiplierProvider(event.target.value) }),
            h('input', { style: inputStyle, placeholder: t('multiplierBps'), value: multiplierBps, onChange: (event) => setMultiplierBps(event.target.value) }),
