...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // closes or a connection action mutates account state.
    ...
    function ConnectionSection({ ctx, store, t, identity, onChanged }) {
      const summary = useAsync(() => ensureAccountSummary(t, ctx, store), [t, ctx, store])
      ...
      // observations) mutate what summary reports: invalidate the shared
      // session first, then refetch through the cache.
      const refreshSummary = () => { invalidateAccountSummary(store); summary.reload() }
      ...
      const waitForLogin = async (providerId, loginId) => {
        for (let attempt = 0; attempt < 120; attempt += 1) {
          ...
          if (status.kind === 'succeeded') { setLogin(null); setConnecting(false); refreshSummary(); onChanged?.(); return }
      ...
      React.useEffect(() => {
        if (reattached || summary.status !== 'ready') return
        setReattached(true)
        const connectionRow = connectionOf(summary.value, identity)
        ...
        void accountRpc(t, ctx, 'connection-action', { provider: providerId, action: 'pending-login', params: { provider: providerId } })
          .then((pending) => {
            ...
            if (found?.loginId) {
              ...
              void waitForLogin(providerId, found.loginId).catch(error => setMessage(error.message))
          ...
          .catch(() => {})
        return () => { live = false }
      }, [reattached, summary.status, summary.value, identity, t, ctx])
            ...
            provider === 'ollama-cloud' ? h('button', {
              ...
              onClick: () => {
                ...
                void accountRpc(t, ctx, 'sync-model-catalog', { providerId: 'ollama-cloud', refresh: true })
                  .then(() => { setMessage(t('cloudModelsSynced')); refreshSummary() })
                  ...
                  h('button', {
                    ...
                    onClick: () => {
                      ...
                      void rpc(t, ctx, 'set-setting', { key: 'ollamaCloudAutoObserve', value: true })
                        ...
                        .then(() => { store.update({ settingsData: { ...(store.state.settingsData ?? {}), ollamaCloudAutoObserve: true } }); refreshSummary() })
