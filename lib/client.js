...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function writeHiddenAccounts(ids) {
      try { localStorage.setItem(HIDDEN_KEY, JSON.stringify(ids.map(String))) } catch { /* unavailable */ }
    }
    ...
    function readWatchedWindows() {
      try {
        const parsed = JSON.parse(localStorage.getItem(WATCHED_KEY) ?? 'null')
    ...
    function writeSidebarWatch(poolId) {
      try {
        if (poolId == null) localStorage.removeItem(WATCH_KEY)
        else localStorage.setItem(WATCH_KEY, poolId)
      } catch { /* storage unavailable */ }
    }
    ...
    function shortError(error, fallback) {
      ...
      if (error?.code === 'session-invalid-or-expired') {
        return { short: '会话 Cookie 无效或过期——请重新从 ollama.com 设置页复制', raw: message }
      }
    ...
    async function setCredential(ctx, ref, value) {
      ...
      if (response?.result?.ok !== true) {
        throw new Error(response?.result?.error?.message ?? 'credential save failed')
      }
      ...
      if (describe?.result?.ok !== true) {
        throw new Error(describe?.result?.error?.message ?? 'credential describe failed')
      }
      if (describe.result.value.credentials?.[ref]?.configured !== true) {
        throw new Error('凭据保存未生效（写入后 describe 未确认）')
      }
    }
    ...
    function sanitizeOllamaCookieHeader(header) {
      ...
      if (selected.length === 0) {
        throw new Error(`未识别到白名单内的 Ollama 会话 Cookie(允许: __Secure-session / session / ollama_session / next-auth…);${unrecognized.length ? `检测到的名称: ${unrecognized.join(', ')}` : '输入不包含 name=value 对——直接粘贴 __Secure-session 的值即可'}`)
      }
    ...
    function DuoMeter({ pool, index = 0, t, compact = false, primaryWin = null }) {
        ...
        if (compact) {
          ...
          return h('div', null,
            h('div', { className: 'tu3-winlab' },
              ...
              h('span', null, primary.resetsAt ? `${t('resetsInLabel')} ${countdown(primary.resetsAt - Date.now(), t)}` : null),
            ),
            h('div', { className: 'tu3-duo-track', style: risk ? { background: C.errorBg } : undefined },
              h('i', { className: 'tu3-duo-fill', style: { width: `${used}%`, background: risk ? C.error : color } }),
            ),
      ...
      return h('div', null,
        ...
        h('div', { className: 'tu3-winlab', style: { marginTop: 4 } },
          h('span', { title: `${windowLabel(top, t)} ${top.usedPct.toFixed(0)}%${secondary ? ` · ${windowLabel(secondary, t)} ${secondary.usedPct.toFixed(0)}%` : ''}` },
            windowLabel(top, t), ' ', h('b', null, `${top.usedPct.toFixed(0)}%`),
    ...
    function ConnectionSection({ ctx, store, t, identity, onChanged }) {
      const summary = useAsync(() => ensureAccountSummary(ctx, store), [ctx, store])
      ...
      const disconnect = async () => {
        ...
        try {
          ...
          const short = shortError(error, '凭据保存失败')
      ...
      const saveCredential = async (kind) => {
        ...
        setMessage(null)
        try {
          if (kind === 'glm') await setCredential(ctx, 'ZAI_CODING_CN_API_KEY', drafts.glm.trim())
            ...
            try {
              ...
              const short = shortError(error, '官方用量观察失败')
    ...
    function AccountInsight({ ctx, store, state, t, compact = false }) {
      ...
      const detail = useAsync(() => rpc(ctx, 'inspect', { kind: 'pool', id, filter: { time: { preset: '30d' } } }), [ctx, id])
          ...
          .then((payload) => {
            ...
            if (failed) {
              const readable = failed.error.code === 'session-invalid-or-expired'
                ? '会话 Cookie 无效或过期——请重新从 ollama.com 设置页复制'
      ...
      const localSection = h('section', { className: 'tu3-card', style: { padding: '14px 16px' } },
        ...
        report.direct.cost.estimatedCacheReadTokens > 0
          ? h('p', { style: { margin: '8px 0 0', fontSize: 11.5, color: C.warning ?? '#b86b00', fontWeight: 600 } }, t('cacheEstimated', {
              rate: (Number(report.direct.cost.cacheEstimateRateBps ?? 0) / 100).toFixed(1).replace(/\.0$/, ''),
        ...
        h('table', { style: tableStyle },
          ...
          h('tbody', null, displayRows.map((account, index) => h('tr', {
            ...
            h('td', { style: { ...tdStyle, whiteSpace: 'nowrap' } },
              ...
              h('button', { type: 'button', style: { ...buttonStyle, padding: '3px 9px', fontSize: 12, marginLeft: 6 }, onClick: () => toggleVisibility(account) },
    ...
    function Dashboard({ ctx, store, state, t }) {
      ...
      React.useEffect(() => { void ensureAccountSummary(ctx, store).catch(() => {}) }, [ctx, store])
      ...
      return h('div', { className: 'tu3-wrap' },
        h('div', { className: 'tu3-hero' },
          h('section', { className: 'tu3-card', style: { padding: '18px 22px' } },
            ...
            state.lens !== 'token' && state.lens !== 'req' && cacheScenarioCost
              ? h('div', { style: { color: C.warning ?? '#b86b00', marginTop: 5, fontSize: 11.5, fontWeight: 600 } }, t('cacheEstimated', {
                  rate: (Number(cacheScenarioCost.cacheEstimateRateBps ?? 0) / 100).toFixed(1).replace(/\.0$/, ''),
        ...
        h('div', { ref: tabsRowRef }, h(AccountTabs, { data, list: orderedPools, state, store, t })),
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 12 } },
                orderedPools.map((pool) => h('div', {
                  ...
                  h('div', { style: { display: 'flex', gap: 14, flexWrap: 'wrap', color: C.textSecondary, fontSize: 12, marginTop: 6 } },
                    h('span', null, `${t('equivalent')} `, h('b', { style: { color: C.text, font: '650 12px ui-monospace, monospace' } }, `${pool.kpis.cost.estimatedCacheReadTokens > 0 ? '≈' : ''}${fmtUsd(pool.kpis.cost.currentUsdNano)}`)),
                    pool.kpis.cost.estimatedCacheReadTokens > 0 ? h('span', {
                      style: { color: C.warning ?? '#b86b00', fontWeight: 600 },
                      title: t('cacheEstimated', {
                        rate: (Number(pool.kpis.cost.cacheEstimateRateBps ?? 0) / 100).toFixed(1).replace(/\.0$/, ''),
                        upper: fmtUsd(pool.kpis.cost.reportedUsageUsdNano),
                      }),
                    }, t('cacheScenarioShort', { rate: (Number(pool.kpis.cost.cacheEstimateRateBps ?? 0) / 100).toFixed(1).replace(/\.0$/, '') })) : null,
        ...
        return h('div', { style: { paddingBottom: 18 } },
          ...
          h('div', { style: { margin: `0 ${S.l}px`, padding: `${S.l}px ${S.s}px ${S.m}px`, borderRadius: 12, background: C.bgMuted, textAlign: 'center' } },
            ...
            state.lens !== 'token' && state.lens !== 'req' && Number(data.kpis.cost.estimatedCacheReadTokens ?? 0) > 0
              ? h('div', { style: { color: C.warning ?? '#b86b00', fontSize: 11.5, fontWeight: 600, marginTop: 5 } }, t('cacheEstimated', {
                  rate: (Number(data.kpis.cost.cacheEstimateRateBps ?? 0) / 100).toFixed(1).replace(/\.0$/, ''),
            ...
            return h('div', {
              ...
              h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: FONT.body, color: C.textSecondary, marginBottom: 6 } },
                ...
                h('span', { style: { display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 } },
                  ...
                  h('b', { style: { color: C.text, font: `650 ${FONT.body}px ui-monospace, monospace` } }, dockRowPct == null ? '—' : `${dockRowPct.toFixed(0)}%`),
                ),
              ),
              h(DuoMeter, { pool, index: poolIndexOf(pool), t, compact: true, primaryWin: dockRowWin }),
            )
            }),
          ),
        )
    ...
    function DataTab(props) {
      ...
      const refresh = React.useCallback(() => (
        ...
        rpc(ctx, 'import-status').then(setStatus).catch(() => {})
      ), [ctx])
    ...
    function SettingsTab(props) {
      ...
      const save = (key, value) => {
        void rpc(ctx, 'set-setting', { key, value })
    ...
    function SidebarEntry(props) {
      ...
      const pctLabel = usedPct != null ? `${usedPct.toFixed(0)}%` : '—'
      const title = t('entryTitle')
      const pinBit = pinned ? '★ ' : ''
      const windowBit = focus?.windowLabel ? ` · ${focus.windowLabel}` : ''
      const resetBit = focus?.resetsAt ? ` · ${t('resetIn')} ${countdown(focus.resetsAt - Date.now(), t)}` : ''
      ...
      const timePart = focus == null
        ...
        : usedPct == null
          ? t('entryNoQuota')
          : focus.resetsAt != null
            ? `${t('resetIn')} ${countdown(focus.resetsAt - Date.now(), t)}`
      ...
      const tightestBit = pinned && summary?.tightest != null && summary.tightest.usedPct != null
        ? `${t('tightestPool')}: ${summary.tightest.name} ${summary.tightest.usedPct.toFixed(0)}%`
        : ''
