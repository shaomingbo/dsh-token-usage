    ...
    function DuoMeter({ pool, index = 0, t, compact = false }) {
      ...
      if (officialWindows.length > 0) {
        if (compact) {
          ...
          return h('div', null,
            h('div', { className: 'tu3-winlab' },
              h('span', null, primary.label ?? primary.externalKey ?? '', source ? ` · ${source}` : ''),
      ...
      const body = () => {
        ...
        return h('div', { style: { paddingBottom: 18 } },
          ...
          tightest && (tightest.pace?.ratePerDay != null || tightest.kind === 'prepaid')
            ? h('div', { style: { margin: '12px 14px 0', padding: '9px 12px', borderRadius: 9, background: C.bgMuted, color: C.textSecondary, fontSize: 11.5 } },
