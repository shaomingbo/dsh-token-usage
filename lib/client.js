...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      accountsTab: 'Accounts', accountsCount: 'accounts', accountAdd: 'Add account', accountNameLabel: 'Name',
    ...
    const zh = {
      ...
      accountsTab: '账户', accountsCount: '个账户', accountAdd: '添加账户', accountNameLabel: '名称',
    ...
    const sourceBadgeStyle = (official) => ({
      ...
      color: official ? 'var(--dsw-alias-state-business-primary, #4176e6)' : C.textSecondary,
    })
    ...
    const poolColor = (pool, index = 0) => pool?.color || (pool?.id === 'unassigned' ? UNASSIGNED_COLOR : POOL_COLORS[index % POOL_COLORS.length])
    ...
    function LensSeg({ state, store, t }) {
      return h('span', { className: 'tu3-seg', role: 'tablist' },
     ...
     */
    function DuoMeter({ pool, index = 0, t }) {
    ...
    // ---------- overview pieces ----------
    function AccountStrip({ pools, lens, t }) {
      return h('div', { className: 'tu3-card' },
        h('div', { className: 'tu3-pools' },
          pools.pools.map((pool, index) => h('div', { key: pool.id },
            h('div', { style: { fontSize: 12, color: C.textSecondary } },
              h('i', { className: 'tu3-dot', style: { background: poolColor(pool, index) } }),
              pool.name,
            ),
    ...
    function AccountTabs({ data, state, store, t }) {
      return h('div', { className: 'tu3-tabs' },
        ...
        data.pools.pools.map((pool, index) => h('button', {
          key: pool.id, type: 'button', className: state.account === pool.id ? 'on' : '',
          onClick: () => store.update({ account: pool.id }),
        }, h('i', { className: 'tu3-dot', style: { background: poolColor(pool, index) } }), pool.name)),
    ...
    function ConnectionSection({ ctx, t, identity, onChanged }) {
      ...
      return h('div', null,
        ...
        provider === 'glm' || provider === 'ollama-cloud' ? h('div', { style: { display: 'grid', gap: 8, marginTop: 10 } },
          provider === 'glm'
      ...
      const submit = async () => {
        ...
        try {
          ...
          await accountRpc(ctx, 'save-account', {
            account: { id: account.id, name, kind, connectionId: account.connectionId, billing, rules },
          })
          onSaved?.()
        } catch (error) {
          setMessage(error.message)
    ...
    function AccountsEditor({ ctx, store, t }) {
      ...
      return h('div', null,
        ...
        h('table', { style: tableStyle },
          ...
          h('tbody', null, rows.map((account, index) => h('tr', { key: account.id, style: account.archived ? { opacity: 0.5 } : undefined },
        ...
        h('div', { className: 'tu3-hero' },
          h('section', { className: 'tu3-card', style: { padding: '18px 22px' } },
            ...
            h('div', { style: { marginTop: 12 } },
              h(LensSeg, { state, store, t }),
            ),
          ),
          h(AccountStrip, { pools, lens: state.lens, t }),
        ),
        h(ActivityChart, { data, state, store, t }),
        h(ModelRank, { data, state, t }),
        h(AccountTabs, { data, state, store, t }),
        state.account === null || state.account === 'unassigned'
          ? h('section', { className: 'tu3-card', style: { padding: '16px 18px' } },
              h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 12 } },
                pools.pools.map((pool, index) => h('div', { key: pool.id, style: { border: `1px solid ${C.borderFaint}`, borderRadius: 12, padding: '13px 15px' } },
    ...
    function DockPanel({ ctx, store, state, t }) {
        ...
        return h('div', { style: { padding: 12 } },
          h('div', { className: 'tu3-card', style: { overflow: 'hidden' } },
            ...
            h('div', { style: { display: 'flex', gap: 5, overflow: 'auto', padding: '8px 12px', borderBottom: `1px solid ${C.borderFaint}` } },
              h('button', { type: 'button', className: `tu3-tab${state.account === null ? ' on' : ''}`, style: { padding: '3px 10px', fontSize: 12 }, onClick: () => store.update({ account: null }) }, t('allPools')),
              pools.pools.map((pool, index) => h('button', {
                key: pool.id, type: 'button', className: `tu3-tab${state.account === pool.id ? ' on' : ''}`,
                style: { padding: '3px 10px', fontSize: 12 }, onClick: () => store.update({ account: pool.id }),
              }, h('i', { className: 'tu3-dot', style: { background: poolColor(pool, index) } }), pool.name.replace(/ (Plus|Pro|Max)$/g, ''))),
            ),
            h('div', { style: { padding: '12px 14px 8px', textAlign: 'center' } },
              ...
              h('div', { style: { color: C.textSecondary, fontSize: 11.5, marginTop: 2 } },
                state.account === null
                  ? `${t('allPools')}${pools.unassigned ? ` + ${t('unassignedPool')}` : ''} · ${lensLabel(t, state.lens)}`
                  : `${pools.pools.find((pool) => pool.id === state.account)?.name ?? state.account}`),
            ),
            h('div', { style: { padding: '4px 14px 12px' } },
              h('div', { className: 'tu3-k', style: { marginBottom: 6 } }, t('poolsMonthly')),
              pools.pools.map((pool, index) => h('div', { key: pool.id, style: { marginBottom: 10 } },
                h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: C.textSecondary, marginBottom: 4 } },
                  h('span', null, h('i', { className: 'tu3-dot', style: { background: poolColor(pool, index) } }), pool.name),
                  h('b', { style: { color: C.text, font: '650 11.5px ui-monospace, monospace' } }, pool.usedPct == null ? '—' : `${pool.usedPct.toFixed(0)}%`),
                ),
                h(DuoMeter, { pool, index, t }),
              )),
            ),
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: `1px solid ${C.border}` } },
          ...
          h('span', { style: { marginLeft: 'auto' } }),
          h('button', { type: 'button', style: { ...buttonStyle, padding: '6px 12px', fontSize: 12 }, onClick: () => store.update({ mode: 'dash', account: null }) }, t('fullscreen')),
    ...
    function PricingTab(props) {
      ...
      const refreshUpstreamPrices = async () => {
        ...
        try {
          ...
          await Promise.all([settings.reload(), catalog.reload()])
          setPriceMessage(t('cloudModelsSynced'))
        } catch (e) {
          setPriceMessage(e.message)
        } finally {
          setPriceBusy(false)
