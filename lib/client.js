...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      unconfiguredBody: 'Define your subscriptions and prepaid balances (quota, reset day, balance, expiry) plus attribution rules — the dashboard then reports objective usage per pool.',
      ...
      sidebarPools: 'Tightest pool', dataCorner: 'Data & settings', backToDash: '← Dashboard',
    }
    ...
    const zh = {
      ...
      unconfiguredBody: '先定义你的订阅与预付余额（额度、重置日、余额、到期日）和归因规则，仪表盘即可按池呈现客观数据。',
      ...
      sidebarPools: '最紧一池', dataCorner: '数据与设置', backToDash: '← 仪表盘',
    }
          ...
          h('div', { style: { display: 'flex', flexDirection: 'column', gap: 10 } },
            ...
            h('label', { style: { display: 'flex', flexDirection: 'column', gap: 4, fontSize: 13 } },
              ...
              h('select', {
                ...
                h('option', { value: 'pools' }, t('sidebarPools')),
    ...
    function DuoMeter({ pool, t }) {
      if (pool.kind === 'credit') {
        ...
        return h('div', null,
          ...
          h('div', { style: { display: 'flex', justifyContent: 'space-between', color: C.textSecondary, fontSize: 12, marginTop: 5 } },
      ...
      return h('div', null,
        ...
        h('div', { className: 'tu3-duo-time' }, h('i', { style: { width: `${Math.min(100, pool.cycle?.elapsedPct ?? 0)}%` } })),
        h('div', { style: { display: 'flex', justifyContent: 'space-between', color: C.textSecondary, fontSize: 12, marginTop: 5 } },
          h('span', null, `${t('quotaUsed')} ${fmtQuota(pool.used, pool.quotaUnit)} / ${fmtQuota(pool.quotaValue, pool.quotaUnit)} (${(pool.usedPct ?? 0).toFixed(0)}%)`),
          h('span', null, `${t('monthProgress')} ${(pool.cycle?.elapsedPct ?? 0).toFixed(0)}%`),
        ),
      )
    }
    ...
    function Dashboard({ ctx, store, state, t }) {
      ...
      if (!data.pools?.configured) return h('div', { className: 'tu3-wrap' }, h(Unconfigured, { store, t }))
    ...
    function DockPanel({ ctx, store, state, t }) {
      ...
      const body = () => {
        ...
        if (!data.pools?.configured) {
          return h('div', { style: { padding: '18px 14px', textAlign: 'center' } },
            h('p', { style: mutedStyle }, t('unconfiguredBody')),
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderBottom: `1px solid ${C.border}` } },
          h('b', { style: { fontSize: 13 } }, t('title')),
          h('span', { style: { fontSize: 11, color: C.textSecondary } }, t('nav')),
          h('span', { style: { marginLeft: 'auto' } }),
          h('button', { type: 'button', style: { ...buttonStyle, padding: '3px 9px', fontSize: 12 }, onClick: () => store.update({ mode: 'dash' }) }, t('fullscreen')),
          h('button', { type: 'button', style: closeStyle, onClick: () => store.update({ open: false }) }, '✕'),
        ),
    ...
    function PlansEditor({ ctx, store, t }) {
      ...
      const blank = { kind: 'sub', name: '', priceUsd: '', quotaUnit: 'newCompute', quotaValue: '', resetDay: 1, balanceUsd: '', expiryDay: '' }
      const startEdit = (plan) => setEditing(plan === null ? { ...blank } : {
        ...
        resetDay: plan.resetDay ?? 1, balanceUsd: plan.balanceUsdNano != null ? String(plan.balanceUsdNano / 1e9) : '',
      ...
      const submit = () => {
        ...
        save({
          ...
          resetDay: Number(draft.resetDay), balanceUsd: draft.balanceUsd === '' ? undefined : Number(draft.balanceUsd), expiryDay: draft.expiryDay || undefined,
        })
        setEditing(null)
      }
      ...
      return h('div', null,
        ...
        h('table', { style: tableStyle },
          ...
          h('tbody', null, rows.map((plan) => h('tr', { key: plan.id, style: plan.archived ? { opacity: 0.5 } : undefined },
            ...
            h('td', { style: tdStyle }, plan.kind === 'sub' ? `${t('resetDay')} ${plan.resetDay}` : `${t('expiry')} ${plan.expiryDay ?? '—'}`),
        ...
        editing !== null ? h('div', { style: { ...cardStyle, marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 } },
          ...
          editing.kind === 'sub' ? h('label', { style: { fontSize: 13, display: 'flex', flexDirection: 'column', gap: 4 } }, t('planQuotaUnit'),
          ...
          editing.kind === 'sub' ? h('label', { style: { fontSize: 13, display: 'flex', flexDirection: 'column', gap: 4 } }, t('planQuota'),
            h('input', { style: inputStyle, value: editing.quotaValue, placeholder: '60000000', onChange: (event) => setEditing({ ...editing, quotaValue: event.target.value }) })) : null,
          editing.kind === 'sub' ? h('label', { style: { fontSize: 13, display: 'flex', flexDirection: 'column', gap: 4 } }, t('resetDay'),
    ...
    function Overlay(props) {
      ...
      if (state.mode === 'dock') {
        return h('div', { style: { position: 'absolute', inset: 0, zIndex: 30, pointerEvents: 'auto', display: 'flex', justifyContent: 'flex-end', background: 'rgba(15,17,21,.14)' }, onClick: (event) => { if (event.target === event.currentTarget) store.update({ open: false }) } },
          h('style', null, v3Css),
          h('div', { style: { width: 396, maxWidth: '92vw', height: '100%', background: C.bgBase, borderLeft: `1px solid ${C.border}`, color: C.text, display: 'flex', flexDirection: 'column' }, role: 'dialog', 'aria-label': t('title'), onClick: (event) => event.stopPropagation() },
    ...
    function SidebarEntry(props) {
      ...
      React.useEffect(() => {
        const load = () => {
          ...
          // Legacy summary modes (today tokens / cost) come from the overview endpoint.
          if (mode === 'tokens' || mode === 'cost') void rpc(ctx, 'overview').then((overview) => store.update({ overview })).catch(() => {})
        }
        load()
      ...
      const showBars = wide && mode === 'pools' && summary?.configured && tightest
      const label = wide ? entryLabel(mode, summary, today, t) : t('nav')
      return h('button', {
        type: 'button',
        style: { ...footerButtonStyle(), alignItems: 'flex-start' },
        'aria-label': `${t('nav')} ${label}`,
        title: t('nav'),
        onClick: () => store.update({ open: true, mode: 'dock' }),
      },
        h('style', null, v3Css),
        h(UsageGlyph),
        h('span', { style: { minWidth: 0, flex: 1 } },
          h('span', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
            h('span', null, t('nav')),
            mode !== 'hidden' && label !== t('nav') ? h('span', { style: { marginLeft: 'auto', font: '650 11px ui-monospace, monospace', color: C.textSecondary } }, label) : null,
          ),
          showBars ? h('span', { className: 'tu3-entry-bars', style: { display: 'block' } },
            h('span', { className: 'tu3-entry-b1', style: { display: 'block' } },
              h('i', { style: { display: 'block', width: `${Math.min(100, tightest.usedPct)}%`, background: tightest.usedPct >= 90 ? 'var(--dsw-alias-state-error-primary, #ec1313)' : 'var(--dsw-alias-state-business-primary, #4176e6)' } })),
            h('span', { className: 'tu3-entry-b2', style: { display: 'block' } },
              h('i', { style: { display: 'block', width: `${Math.min(100, summary.month.elapsedPct)}%` } })),
            h('span', { className: 'tu3-entry-cap', style: { display: 'flex' } },
              h('span', null, `${t('tightestPool')}: ${tightest.name} ${tightest.usedPct.toFixed(0)}%`),
              h('span', null, `${t('resetIn')} ${summary.month.daysLeft}${t('dayUnit')}`),
            ),
          ) : null,
        ),
      )
    ...
    function entryLabel(mode, summary, today, t) {
      if (mode === 'hidden' || !summary) return t('nav')
      if (mode === 'cost') return fmtUsd(today?.costUsdNano ?? null)
      if (mode === 'tokens') return today !== undefined && today ? fmtTokens(today.processingTokens ?? 0) : t('nav')
      if (mode === 'pools') return summary.configured && summary.tightest ? `${summary.tightest.usedPct.toFixed(0)}%` : t('nav')
      return t('nav')
    }
