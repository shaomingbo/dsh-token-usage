...
const DEFAULTS = {
  ...
  sidebarSummary: 'pools', // 'pools' level meter | 'badge' | 'plain' | 'hidden' (legacy 'tokens'/'cost' map to plain)
  retentionDays: null, // null = keep forever
  ollamaCloudCacheEstimatePct: 95, // scenario only; real reported cache data wins
  // Persisted consent for background Ollama Cloud Usage scraping while the GUI
  // is open. A stored cookie alone is not authorization.
  ollamaCloudAutoObserve: false,
}
