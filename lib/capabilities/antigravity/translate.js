...
const TIERED_GEMINI_RUNTIMES = new Set(['gemini-3.7-flash-tiered', 'gemini-3.8-flash-tiered'])
...
export function buildGenerateRequest({
  ...
  generationConfig.maxOutputTokens = typeof maxTokens === 'number' && Number.isFinite(maxTokens)
    ...
    : cap
  // Tiered runtimes carry the effort as a thinkingLevel; the fallback ladder can
  // land on the 3.7 tiered id even for a Gemini 3.8 request, so this check runs first.
  if (TIERED_GEMINI_RUNTIMES.has(runtimeModel)) {
    generationConfig.thinkingConfig = {
      thinkingLevel: reasoningEffort === 'high' ? 'HIGH' : reasoningEffort === 'medium' ? 'MEDIUM' : 'LOW',
    }
  } else if (publicModelId.startsWith('gemini-3.8-flash') || runtimeModel.startsWith('gemini-3.8-flash')) {
    // Gemini 3.8 Flash carries its thinking budget on the wire (pi v0.7.2), matching
    // the budgets the backend advertises per variant: low 1000 / medium 4000 / high dynamic.
    generationConfig.thinkingConfig = !reasoningEffort || reasoningEffort === 'off'
      ? { includeThoughts: false, thinkingBudget: 0 }
      : reasoningEffort === 'high' || reasoningEffort === 'xhigh'
        ? { includeThoughts: true, thinkingBudget: -1 }
        : reasoningEffort === 'medium'
          ? { includeThoughts: true, thinkingBudget: 4000 }
          : { includeThoughts: true, thinkingBudget: 1000 }
  }
