...
/** Hard deadline for Google token-endpoint calls; a hung refresh must never park a capability call forever. */
export const REFRESH_TIMEOUT_MS = 15_000
/** Hard deadline for the best-effort userinfo lookup during login. */
...
export class AntigravityAuth {
  ...
  /** The active pending sign-in, for clients reattaching after a remount.
   * The authorization URL is rebuilt from the stored PKCE pair: same
   * verifier/state, so the still-running callback server accepts it. */
  pendingLogin() {
    this.assertOpen()
    const pending = [...this.logins.values()].find(operation => operation.status.kind === 'pending')
    if (pending === undefined) return { active: false, login: null }
    return {
      active: true,
      login: {
        loginId: pending.loginId,
        provider: PROVIDER_ID,
        authUrl: createAuthUrl({ challenge: pending.challenge, state: pending.state }),
        redirectUri: REDIRECT_URI,
      },
    }
  }
  ...
  /**
   * Refresh one account's tokens. Always bounded by REFRESH_TIMEOUT_MS and
   * deliberately independent of any caller's signal: the refresh is shared
   * per account, so one waiter cancelling (say, an observation round hitting
   * its deadline) must not abort the token a concurrent model request is
   * waiting for. Callers that were cancelled simply ignore the result.
   */
