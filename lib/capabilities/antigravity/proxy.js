...
const CONNECTION_ID_HEADER = 'X-DSH-Connection-Id'
...
export function createProxy({ auth, accountRouter, client, logger = { warn: () => {}, info: () => {} }, port = 51122, host = '127.0.0.1', emptyRetryDelays = EMPTY_RETRY_DELAYS_MS } = {}) {
  ...
  function writeStreamHeaders(res, connectionId) {
    if (res.headersSent) return
    res.writeHead(200, {
      'Content-Type': 'text/event-stream',
      'Cache-Control': 'no-store',
      Connection: 'keep-alive',
      ...(connectionId === undefined ? {} : { [CONNECTION_ID_HEADER]: connectionId }),
    })
  }
  ...
  async function streamAttempt({ response, res, publicModelId, connectionId }) {
    ...
    try {
      while (true) {
        ...
        if (frames.length > 0) {
          writeStreamHeaders(res, connectionId)
    ...
    if ((received || upstreamError !== undefined) && !res.writableEnded) {
      writeStreamHeaders(res, upstreamError === undefined ? connectionId : undefined)
      res.write(tail.join(''))
    }
  ...
  async function handleChatCompletions(req, res) {
    ...
    if (!wantsStream) {
      ...
      res.writeHead(200, {
        ...
        'Cache-Control': 'no-store',
        [CONNECTION_ID_HEADER]: outcome.accountId,
      })
    ...
    let attemptCount = 0
    let result = await streamAttempt({ response: streamingResponse, res, publicModelId: params.model, connectionId: outcome.accountId })
    while (result.empty && attemptCount < emptyRetryDelays.length && !closed) {
      ...
      if (!retry.ok) break
      result = await streamAttempt({ response: retry.response, res, publicModelId: params.model, connectionId: outcome.accountId })
    }
    if (result.empty) {
