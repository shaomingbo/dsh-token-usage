...
import { attributedResponseId } from './response-provenance.js'
...
export function createProxy({ auth, accountRouter, client, logger = { warn: () => {}, info: () => {} }, port = 51122, host = '127.0.0.1', emptyRetryDelays = EMPTY_RETRY_DELAYS_MS } = {}) {
  ...
  async function streamAttempt({ response, res, publicModelId, connectionId }) {
    const translator = createStreamTranslator({
      model: publicModelId,
      id: attributedResponseId(connectionId),
    })
  ...
  async function handleChatCompletions(req, res) {
    ...
    if (!wantsStream) {
      const collector = createStreamTranslator({
        model: params.model,
        id: attributedResponseId(outcome.accountId),
      })
      const decoder = new TextDecoder()
