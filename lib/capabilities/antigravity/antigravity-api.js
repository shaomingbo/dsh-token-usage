...
/** Default bound for every JSON API call, so one hung connection can never leave a request pending forever. */
export const DEFAULT_POST_TIMEOUT_MS = 10_000
...
/** Merge an optional caller signal with a hard per-request deadline. */
function boundedSignal(signal, timeoutMs) {
  const deadline = AbortSignal.timeout(timeoutMs)
  if (signal === undefined || typeof AbortSignal.any !== 'function') return deadline
  return AbortSignal.any([signal, deadline])
}

/** `POST` JSON and fold non-OK statuses into thrown, bounded errors. */
async function postJson(fetchImpl, url, headers, body, timeoutMs = DEFAULT_POST_TIMEOUT_MS, signal) {
...
export function createAntigravityClient({ fetchImpl = fetch, clock = Date.now, baseUrl } = {}) {
  ...
  async function loadCodeAssist(token, { signal } = {}) {
    ...
    for (const endpoint of endpointCandidates()) {
      try {
        ...
        }, DISCOVERY_TIMEOUT_MS, signal)
        if (!result.ok) continue
        projectId = extractProjectId(result.data)
        if (projectId) break
        const listed = await postJson(fetchImpl, `${endpoint}/v1internal:listCloudAICompanionProjects`, antigravityHeaders(token), {}, DISCOVERY_TIMEOUT_MS, signal)
        if (listed.ok) {
   ...
   */
  async function fetchAvailableModels(token, projectId, { signal } = {}) {
    for (const endpoint of endpointCandidates()) {
      try {
        const result = await postJson(fetchImpl, `${endpoint}/v1internal:fetchAvailableModels`, antigravityHeaders(token), { project: projectId }, DISCOVERY_TIMEOUT_MS, signal)
        if (!result.ok) continue
  ...
  async function retrieveUserQuotaSummary(token, { signal, timeoutMs = DEFAULT_POST_TIMEOUT_MS } = {}) {
    const result = await postJson(fetchImpl, `${DEFAULT_ENDPOINT}/v1internal:retrieveUserQuotaSummary`, antigravityHeaders(token), {}, timeoutMs, signal)
    if (!result.ok) {
   ...
   *
   * Deliberately no absolute timeout: a generate call streams for as long as
   * the model needs, and an `AbortSignal.timeout` would truncate live
   * generations mid-stream. The bound is the caller's request `signal`
   * (the proxy passes one per client request), which aborts both the fetch
   * and the body read.
   */
