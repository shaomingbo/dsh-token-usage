...
/** Quota field names the usage layer can read off a model entry itself. */
const FLAT_QUOTA_KEYS = ['remainingFraction', 'resetTime', 'remaining', 'quotaRemaining']

/**
 * Merge quota facts across surfaces without changing the catalog: the first
 * answering surface owns the model set and every quota field it reports;
 * later surfaces only fill quota fields it omitted (verified 2026-09-13:
 * daily answers some consumer accounts without remainingFraction for the
 * gemini-* runtimes while prod still reports the real fractions with the
 * same fixed reset instant). Entries the filler does not know pass through
 * untouched, and a model carrying its quota fields flat never gains a
 * nested quotaInfo that would shadow them.
 */
function mergeModelQuota(primary, filler) {
  const merged = {}
  for (const [id, info] of Object.entries(primary)) {
    if (!isRecord(info) || !isRecord(filler[id])) {
      merged[id] = info
      continue
    }
    const fill = filler[id]
    const quota = isRecord(info.quotaInfo) ? { ...info.quotaInfo } : undefined
    const fillQuota = isRecord(fill.quotaInfo) ? fill.quotaInfo : undefined
    if (quota === undefined) {
      const hasOwnQuota = FLAT_QUOTA_KEYS.some(key => info[key] !== undefined)
      merged[id] = fillQuota === undefined || hasOwnQuota ? info : { ...info, quotaInfo: { ...fillQuota } }
      continue
    }
    if (fillQuota !== undefined) {
      for (const key of ['remainingFraction', 'resetTime']) {
        if (quota[key] === undefined && fillQuota[key] !== undefined) quota[key] = fillQuota[key]
      }
    }
    merged[id] = { ...info, quotaInfo: quota }
  }
  return merged
}

/** True when every catalog entry already reports a numeric remaining fraction. */
function modelsQuotaComplete(models) {
  const entries = Object.entries(models)
  return entries.length > 0 && entries.every(([, info]) => {
    const quota = isRecord(info) && isRecord(info.quotaInfo) ? info.quotaInfo : info
    return typeof quota?.remainingFraction === 'number'
  })
}
...
export function createAntigravityClient({ fetchImpl = fetch, clock = Date.now, baseUrl } = {}) {
  ...
  /**
   * Wire fingerprint aligned with the official agy CLI (pi-antigravity v0.7.2,
   * PR #35): only Authorization, Content-Type, and the CLI User-Agent. The
   * legacy VS Code extension headers (X-Goog-Api-Client, Client-Metadata) are
   * what Google's server used to classify this token's surface into the old
   * 25-model catalog — with them, Gemini 3.8 was gated out of discovery and
   * generation entirely (verified 2026-09-05).
   */
  function antigravityHeaders(token) {
    return {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
      'User-Agent': process.env.DSH_ANTIGRAVITY_USER_AGENT?.trim() || 'antigravity/cli/1.1.23 (aidev_client; os_type=linux; arch=amd64; cl=974125021; auth_method=consumer)',
    }
  }
  ...
  /**
   * Quota and catalog reads: consumer usage state lives on the daily
   * generation surface. Prod answers these requests with 200 + placeholder
   * quota (remainingFraction=1, resetTime=now+5h/7d) — verified 2026-09-08 —
   * so polling it first persisted an all-0% echo with a never-converging ~5h
   * countdown. Prefer daily, keep prod and sandbox as fallbacks; an explicit
   * base URL still pins every operation for gateways/tests. The placeholder
   * echo itself is not endpoint-bound: the claude-* and gpt-oss rows answer the
   * same echo on every surface (verified 2026-09-13), which the usage layer
   * detects instead of trusting any single surface.
   */
  function quotaEndpointCandidates() {
    const explicit = baseUrl?.trim() || process.env.DSH_ANTIGRAVITY_BASE_URL?.trim()
    return explicit ? [explicit] : [DEFAULT_GENERATION_ENDPOINT, DEFAULT_ENDPOINT, 'https://daily-cloudcode-pa.sandbox.googleapis.com']
  }
   ...
   * Reads the quota-bearing surfaces in turn and merges quota facts: the
   * first answering surface owns the model set and every field it reports,
   * later surfaces only fill quota fields it omitted, and the walk stops as
   * soon as every entry reports a numeric remaining fraction (a complete
   * answer costs exactly one request, as before). Returns undefined when no
   * endpoint answered.
   */
  async function fetchAvailableModels(token, projectId) {
    let merged
    for (const endpoint of quotaEndpointCandidates()) {
      try {
        ...
        if (!result.ok || !isRecord(result.data) || !isRecord(result.data.models)) continue
        merged = merged === undefined ? result.data.models : mergeModelQuota(merged, result.data.models)
        if (modelsQuotaComplete(merged)) return merged
      } catch {
        // try the next endpoint
      }
    }
  ...
  /**
   * User-quota summary. Reads the quota-bearing surface first (see
   * quotaEndpointCandidates): prod answers 200 with placeholder buckets for
   * consumer subscriptions, so it is a fallback here, not the source.
   */
  async function retrieveUserQuotaSummary(token) {
    let last
    for (const endpoint of quotaEndpointCandidates()) {
      const result = await postJson(fetchImpl, `${endpoint}/v1internal:retrieveUserQuotaSummary`, antigravityHeaders(token), {})
      ...
      last = { status: result.status, message }
    }
    throw new Error(`retrieveUserQuotaSummary failed (${last.status}): ${last.message}`)
  }
