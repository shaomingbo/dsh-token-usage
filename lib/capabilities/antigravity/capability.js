...
const envelope = createCapabilityEnvelope({
  cancelledCodes: ['ANTIGRAVITY_LOGIN_ABORTED'],
  credentialCodes: ['ANTIGRAVITY_AUTH_EXPIRED', 'ANTIGRAVITY_AUTH_NOT_CONFIGURED'],
})
export const antigravityEnvelopeOutcome = envelope.outcome
...
export async function ensureAntigravityRoute(settings, proxyUrl, logger = {}) {
  const snapshot = settings.read(ANTIGRAVITY_ROUTE_ID)
  if (!snapshot) return false
  const existing = snapshot.value
  if (!antigravityRouteNeedsProvisioning(existing, proxyUrl)) return false
  // connectionIdHeader is no longer in the host schema. Do not send an unset
  // for a non-schema field (nor assume unset removes an inherited base value).
  if (Object.hasOwn(existing ?? {}, 'connectionIdHeader')) {
    logger.warn?.('provider-capability: obsolete connectionIdHeader requires explicit profile cleanup')
    return false
  }
  const patch = antigravityRoutePatch(existing, proxyUrl)
  // Never replay existing model entries read through secret redaction.
  if (Array.isArray(existing?.models) && existing.models.length > 0) delete patch.models
  ...
  async function handleRpc(endpoint, payload, signal) {
    try {
      ...
      if (endpoint === 'pending-login') {
        requireObject(payload)
        return success(auth.pendingLogin())
      }
      ...
      if (endpoint === 'diagnostics') return success({
        ...
        proxyUrl,
        routeConfigured: settings?.read(ANTIGRAVITY_ROUTE_ID)?.value !== undefined,
      })
