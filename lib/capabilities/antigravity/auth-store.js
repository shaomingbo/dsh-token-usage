 ...
 * queue, and last-good-snapshot survival come from the shared owner-file
 * kernel, as in the pi-ai credential store.
 */
...
export class AuthStore {
  constructor({ filename, onError = () => {}, onChanged = () => {} } = {}) {
    ...
    this.files = new OwnerFileStore({
      path: filename,
      parse: parseAuthDocument,
      onInvalid: error => this.onError(error),
    })
  }
  ...
  async modifyAccount(accountId, mutator) {
    assertAccountId(accountId)
    if (this.closed) return this.accounts.get(accountId)
    return this.files.enqueue(async () => {
      const current = this.accounts.get(accountId)
  ...
  async writeState({ accounts, activeAccountId, autoFailover }) {
    ...
    await this.files.commit(document)
  }
