...
export class SubscriptionAuthRuntime {
  ...
  loginStatus(loginId) {
    ...
    return operation.status.kind === 'pending' && operation.challengeValue
      ? { ...operation.status, challenge: { ...operation.challengeValue } }
      : operation.status
  }
  ...
  pendingLogin(provider) {
    this.assertOpen()
    const id = authProviderId(provider)
    const operation = this.logins.get(this.providerLogins.get(id))
    if (!operation || operation.status.kind !== 'pending') return { active: false, login: null }
    return {
      active: true,
      login: operation.challengeValue
        ? { ...operation.challengeValue }
        : { loginId: operation.loginId, provider: id },
    }
  }

  async cancelProviderLogin(provider) {
    this.assertOpen()
    const id = authProviderId(provider)
  ...
  receiveAuthEvent(operation, event) {
    ...
    operation.challenge.resolve({ ...operation.challengeValue })
  }
