...
export function sanitizeUsage(value) {
  ...
  if (value.available !== true) {
    const unavailable = { provider: value.provider, available: false }
    const error = sanitizeError(value.error)
    if (error !== undefined) unavailable.error = error
    const checkedAt = finiteNumber(value.checkedAt)
    if (checkedAt !== undefined) unavailable.checkedAt = checkedAt
    return unavailable
  }
  const windows = []
  ...
  const checkedAt = finiteNumber(value.checkedAt)
  if (checkedAt !== undefined) result.checkedAt = checkedAt
  const error = sanitizeError(value.error)
  if (error !== undefined) result.error = error
  const retryAfterMs = finiteNumber(value.retryAfterMs)
...
async function requestUsage(fetchImpl, url, headers, signal) {
  ...
  if (!response.ok) {
    const error = usageError('USAGE_UNAVAILABLE', 'Usage is temporarily unavailable')
    const retry = retryAfterMs(response)
    if (retry != null) error.retryAfterMs = retry
    throw error
  }
...
/**
 * Fold a failed refresh into a reportable usage snapshot.
 *
 * A failure never refreshes the data's timestamp: with a previous snapshot the
 * windows keep their original `fetchedAt` (the data's real age), `stale` marks
 * them as not freshly confirmed, and `checkedAt` records when this attempt
 * happened — the host skips persisting stale entries, so an old percentage can
 * no longer be re-filed as a brand-new observation. Without a previous
 * snapshot there is nothing to stand on, so the entry reports itself plainly
 * unavailable instead of claiming availability with empty windows.
 */
function failed(provider, error, previous, now) {
  const previousWindows = Array.isArray(previous?.windows) ? previous.windows : []
  ...
  if (Number.isFinite(error.retryAfterMs)) stale.retryAfterMs = error.retryAfterMs
  return sanitizeUsage(stale)
}
