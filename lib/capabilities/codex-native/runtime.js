...
import { nativeModelFacts } from './catalog.js';
...
export function createCodexRuntime({ resolveOAuth, configured, ready = true, fetchImpl = globalThis.fetch,
  ...
  // Owner-verified ids beyond the pinned set (gpt-6-sol / gpt-6-luna) join the
  // same catalog through the whitelisted custom-model merge, so models() and
  // resolveModel() serve one set of facts. Pinned entries are never replaced
  // and the catalog is not swapped for a newer SDK directory.
  for (const id of ['gpt-6-sol', 'gpt-6-luna']) {
    if (catalog.some(model => model.id === id)) continue;
    const facts = nativeModelFacts(id);
    if (facts) catalog.push(customModel(facts, id, template));
  }
  ...
  async function open({ model, signal, purpose = 'request' } = {}) {
    ...
    function stop(reason, timeoutKind) {
      ...
      // Record an active SDK stream's outcome before stop freezes observation.
      // NativeTransport keeps its existing last-observed phase diagnostics.
      if (active?.mode === 'stream') diagnostics.observe('stream-terminal', reason === 'TIMEOUT' ? 'timed-out' : 'cancelled');
      diagnostics.stop();
    ...
    function provider({ mode = 'stream', replay = [], retentionHints } = {}) {
      requireOpen();
      requireValue(mode === 'stream' || mode === 'compact', 'MODE');
      // Optional source-aware retention envelope (metadata-only: categories
      // and verification digests, never message text). Malformed hints refuse
      // the provider rather than degrading into a guessed policy.
      if (retentionHints !== undefined) {
        requireValue(mode === 'compact' && retentionHints && typeof retentionHints === 'object' && !Array.isArray(retentionHints)
          && retentionHints.version === 1 && retentionHints.algorithm === 'source-aware-v1'
          && Array.isArray(retentionHints.items), 'RETENTION_HINTS');
        for (const hint of retentionHints.items) {
          requireValue(hint && typeof hint === 'object' && !Array.isArray(hint)
            && ['user-instruction', 'system-constraint', 'host-notice', 'checkpoint-wrapper', 'unknown'].includes(hint.category)
            && typeof hint.digest === 'string' && /^[0-9a-f]{64}$/.test(hint.digest), 'RETENTION_HINTS');
        }
      }
      ...
      function streamSimple(_callerModel, context, options = {}) {
        ...
        const flight = { mode }; active = flight;
        // Classify before scheduling, including an already-aborted caller.
        if (mode === 'stream') diagnostics.observe('prepare', mode);
        ...
        const run = async () => {
          ...
          if (mode === 'compact') diagnostics.observe('prepare', mode);
          ...
          if (mode === 'compact') {
            ...
            for await (const event of upstream) {
              ...
              if (event.type === 'done') { diagnostics.observe('completed'); terminal = true; release(); }
              output.push(safe);
              if (terminal) break;
            }
        ...
        void race(callSignal, run).catch(() => {
          if (active === flight && mode === 'stream') diagnostics.observe('stream-terminal',
            stopReason === 'TIMEOUT' ? 'timed-out' : callSignal.aborted ? 'cancelled' : 'failed');
  ...
  return Object.freeze({ protocol: PROTOCOL, describe: () => ({ protocol: PROTOCOL, route: ROUTE,
    ...
    nativeCompaction: true, nativeReplay: true,
    // Capability detection for consumers: an old owner without this marker
    // must keep receiving no hints (its original retention policy applies).
    retentionHints: { supported: true, algorithm: 'source-aware-v1', version: 1 },
    usage: { normal: 'sdk', compact: 'upstream-if-present', estimate: 'native-replay-json-utf16/4' } }),
