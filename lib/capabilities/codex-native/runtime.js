...
export function createCodexRuntime({ resolveOAuth, configured, ready = true, fetchImpl = globalThis.fetch,
  timeoutMs = 30_000, modelProvider, attribution, resolveModelFacts, routeStatus } = {}) {
  ...
  async function open({ model, signal } = {}) {
    requireValue(!disposed, 'DISPOSED');
    const id = typeof model === 'string' ? model : model?.id;
    requireValue(typeof id === 'string' && ID_PATTERN.test(id), 'UNKNOWN_MODEL');
    const controller = new AbortController();
    let credentials, closed = false, expired = false, active;
    let compactUsage = { kind: 'unavailable' };
    const externalAbort = () => close();
    const timer = setTimeout(() => { expired = true; close(); }, timeoutMs);
    function close() {
      if (closed) return;
      closed = true; credentials = undefined; compactUsage = { kind: 'unavailable' }; clearTimeout(timer);
    ...
    function provider({ mode = 'stream', replay = [] } = {}) {
      ...
      function streamSimple(_callerModel, context, options = {}) {
        ...
        const run = async () => {
          requireValue(!closed && !disposed && auth, 'CLOSED');
          ...
          if (mode === 'compact') {
            ...
            const transport = new NativeTransport({ fetch: ownFetch, auth: async () => auth, identity: binding.identity, timeoutMs });
            const result = await transport.compact({ ...binding, input, tools, instructions: inputContext.systemPrompt ?? '', signal: callSignal });
            requireValue(!callSignal.aborted, 'CANCELLED');
        ...
        void race(callSignal, run).catch(() => {
          ...
          failed.errorMessage = `CODEX_RUNTIME_${expired ? 'TIMEOUT' : callSignal.aborted ? 'CANCELLED' : failureCategory}`;
      ...
      return Object.freeze({ id: PROVIDER, name: 'Owner-bound Codex Native', baseUrl: BASE,
        auth: Object.freeze({ apiKey: Object.freeze({ name: 'Owner-bound connection', resolve: async () => {
          requireValue(!closed && !disposed, 'CLOSED'); return { auth: {} };
        } }) }),
    ...
    return Object.freeze({ binding, provider, close,
