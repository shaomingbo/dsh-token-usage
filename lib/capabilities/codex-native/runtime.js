...
export function createCodexRuntime({ resolveOAuth, configured, ready = true, fetchImpl = globalThis.fetch,
  timeoutMs = 30_000, compactionTimeoutMs = timeoutMs, modelProvider, attribution, resolveModelFacts, routeStatus } = {}) {
  ...
  async function open({ model, signal, purpose = 'request' } = {}) {
    ...
    let credentials, closed = false, stopReason, active;
    let compactUsage = { kind: 'unavailable' };
    const externalAbort = () => stop('CANCELLED');
    const timer = setTimeout(() => stop('TIMEOUT'), leaseTimeoutMs);
    function stop(reason) {
      if (closed) return;
      diagnostics.stop();
      closed = true; stopReason = reason; credentials = undefined; compactUsage = { kind: 'unavailable' }; clearTimeout(timer);
      ...
      function streamSimple(_callerModel, context, options = {}) {
        ...
        const run = async () => {
          ...
          if (mode === 'compact') {
            ...
            const transport = new NativeTransport({ fetch: ownFetch, auth: async () => auth, identity: binding.identity, timeoutMs: leaseTimeoutMs, observe: diagnostics.observe });
            let result;
            try {
