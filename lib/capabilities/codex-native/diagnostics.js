...
export function operationDiagnostics(now = () => performance.now()) {
  ...
  function observe(kind, value) {
    ...
    switch (kind) {
      ...
      case 'prepare':
        sdkStream = value === 'stream'; data.phase = 'request-prepare';
        if (sdkStream) delete data.completedMs;
        break;
      case 'request':
      ...
      case 'stream-terminal':
        if (sdkStream && ['failed', 'cancelled', 'timed-out'].includes(value)) data.phase = value;
        break;
    }
  }
  function snapshot() {
    const result = { ...data, eventCounts: { ...data.eventCounts }, elapsedMs: stopped ?? elapsed() };
    // The pinned SDK owns stream body reads and wire-event parsing. Normalized
    // SDK events cannot substantiate byte/chunk/wire-event counters or timings.
    if (sdkStream) for (const key of ['responseBytes', 'chunks', 'events', 'eventCounts', 'firstByteMs', 'lastByteMs', 'lastEventMs', 'lastEvent', 'itemMs']) delete result[key];
    return result;
  }
  return Object.freeze({ observe, stop: () => { stopped ??= elapsed(); }, snapshot });
}
