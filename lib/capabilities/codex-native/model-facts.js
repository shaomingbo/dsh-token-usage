...
export function createCodexModelFacts({ getRoute, resolveModelInfo, credentialRef }) {
  if (typeof getRoute !== 'function' || typeof credentialRef !== 'string' || !credentialRef) {
    throw new Error('createCodexModelFacts requires getRoute() and the openai-codex credential ref');
  }
  const routeStatus = () => {
    let route;
    try { route = getRoute('openai-codex'); } catch { return { ok: false, reason: 'ROUTE_MISSING' }; };
