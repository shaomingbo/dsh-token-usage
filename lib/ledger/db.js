...
const MIGRATIONS = new Map([
  ...
  [9, (db) => {
    // Cache presence is a fact separate from a numeric zero. Legacy rows lost
    // that distinction and remain unknown; positive historical categories are
    // known to have been reported. New import/live paths preserve presence.
    ensureColumn(db, 'requests', 'cache_read_state', "TEXT NOT NULL DEFAULT 'unknown'")
    ensureColumn(db, 'requests', 'cache_write_state', "TEXT NOT NULL DEFAULT 'unknown'")
    db.exec(`
      UPDATE requests SET cache_read_state = 'reported' WHERE cache_read_tokens > 0;
      UPDATE requests SET cache_write_state = 'reported' WHERE cache_write_tokens > 0;
      INSERT OR REPLACE INTO meta (key, value) VALUES ('schema_version', '9');
    `)
  }],
  [10, (db) => {
    // Observations gain a materialized usability marker so latest-read and
    // retention stay O(connections) instead of parsing every historical
    // payload. usable mirrors the read path's old skip condition: 1 when the
    // payload carries at least one limit entry, 0 for reachability probes
    // with no windows. A null percent is still a window (unknown, not
    // absent), so only truly window-less payloads are unusable. Every stored
    // payload is JSON.stringify output; json_valid keeps a corrupted row from
    // aborting the migration.
    ensureColumn(db, 'account_observations', 'usable', 'INTEGER NOT NULL DEFAULT 0')
    db.exec(`
      UPDATE account_observations SET usable = 1
      WHERE json_valid(payload_json) AND json_array_length(payload_json, '$.limits') > 0;
      CREATE INDEX IF NOT EXISTS account_observations_usable_latest
        ON account_observations(connection_id, usable, observed_at DESC);
      INSERT OR REPLACE INTO meta (key, value) VALUES ('schema_version', '10');
    `)
  }],
  [11, (db) => {
    if (db.prepare("PRAGMA table_info('requests')").all().some(row => row.name === 'attempt')) {
      db.exec("INSERT OR REPLACE INTO meta (key, value) VALUES ('schema_version', '11')")
      return
    }
    // Legacy rows retain attempt=0 and their existing IDs/corrections. V4
    // settlements use durable seq+1, not a process-local retry counter.
    const requestSchema = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='requests'").get().sql
    const correctionSchema = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='request_corrections'").get().sql
    const indexes = db.prepare("SELECT sql FROM sqlite_master WHERE type='index' AND tbl_name IN ('requests', 'request_corrections') AND sql IS NOT NULL").all()
    db.exec('CREATE TEMP TABLE saved_corrections AS SELECT * FROM request_corrections')
    db.exec('DROP TABLE request_corrections')
    db.exec(requestSchema.replace(/CREATE TABLE (?:IF NOT EXISTS )?requests/i, 'CREATE TABLE requests_v11')
      .replace(/PRIMARY KEY\s*\(session_id, turn, step\)/i, 'attempt INTEGER NOT NULL DEFAULT 0, PRIMARY KEY (session_id, turn, step, attempt)'))
    const columns = db.prepare("PRAGMA table_info('requests')").all().map(row => `"${row.name}"`).join(', ')
    db.exec(`INSERT INTO requests_v11 (${columns}) SELECT ${columns} FROM requests`)
    db.exec('DROP TABLE requests; ALTER TABLE requests_v11 RENAME TO requests')
    db.exec(correctionSchema.replace(/FOREIGN KEY \(session_id, turn, step\) REFERENCES requests\(session_id, turn, step\)/,
      'attempt INTEGER NOT NULL DEFAULT 0, FOREIGN KEY (session_id, turn, step, attempt) REFERENCES requests(session_id, turn, step, attempt)'))
    const correctionColumns = db.prepare("PRAGMA table_info('saved_corrections')").all().map(row => `"${row.name}"`).join(', ')
    db.exec(`INSERT INTO request_corrections (${correctionColumns}) SELECT ${correctionColumns} FROM saved_corrections`)
    db.exec('DROP TABLE saved_corrections')
