...
export function createLedgerService({
  ...
  return {
    ...
    ensureConnectionAccount(connection, { aliases = [], attributable = true } = {}) {
      ...
      if (attributable) {
        const ruleCount = Number(db.prepare('SELECT COUNT(*) AS n FROM account_attribution_rules WHERE product_id = ?').get(productId).n)
        if (ruleCount === 0) {
          ...
          `)
          for (const alias of (aliases.length > 0 ? aliases : [connection.providerId])) {
            insert.run(productId, `${alias}*`)
          }
          changed = true
        }
      }
