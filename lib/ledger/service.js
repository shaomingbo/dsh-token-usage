...
import { foldAttempts, isAttemptSettlement, parseRequestId } from './attempts.js'
import { serializePublic, normalizeAnchor } from '../accounts/domain.js'
...
/** Exact inherited prefix length. V3 uses inheritedEventCount; V2 used seedLength. */
export function inheritedPrefixLength(header, session) {
  const candidates = [session?.inheritedEventCount, header?.inheritedEventCount, header?.seedLength]
...
function requireOwnershipCut(header, session) {
  const cut = inheritedPrefixLength(header, session)
  if (cut !== null) return cut
  if (header?.isSeeded === true) {
    throw new LedgerError('missing-inherited-cut', `fork session ${header.id} is missing inheritedEventCount/seedLength; refusing to mark inherited usage as owned`)
  }
  return 0
}

/**
 * Official observation history kept per connection. Every refresh files a new
 * row, so an unbounded history grows linearly forever (~1,400 rows/day in the
 * field); retention caps each connection at its newest rows. The newest
 * usable observation is always kept even when it ranks outside this window,
 * so a run of window-less reachability probes can never mask the last real
 * scrape (see the retention statement below).
 */
export const OBSERVATION_RETENTION_PER_CONNECTION = 1000
...
/**
 * Write-side mirror of the v10 migration backfill
 * (`json_array_length(payload_json, '$.limits') > 0`): an observation is
 * usable when its payload carries at least one limit entry. Reachability
 * probes with no windows are not. A limit with a null percent is still a
 * window (unknown, not absent), so it stays usable — matching the official
 * read path, which renders it with a null value.
 */
function observationIsUsable(observation) {
  return Array.isArray(observation.limits) && observation.limits.length > 0
}

/** The optional calendar anchor stored in a limit's window_json column. */
function windowAnchor(windowJson) {
  if (windowJson == null) return null
  try {
    const anchor = JSON.parse(windowJson)?.anchor
    return anchor === undefined ? null : normalizeAnchor(anchor)
  } catch {
    return null
  }
}
...
export function foldEvents(header, events, { estimator } = {}) {
  const records = foldAttempts(header, events, { estimator })
  const attemptSteps = new Set([...records.values()].map(record => `${record.turn}:${record.step}`))
  let pendingConfig
  ...
  const recordFor = (turn, step) => {
    ...
    if (record === undefined) {
      record = {
        ...
        cache_read_state: 'unknown', cache_write_state: 'unknown',
  ...
  const applyUsage = (record, usage, event) => {
    ...
    record.cache_read_state = Object.hasOwn(usage, 'cacheReadTokens') ? 'reported' : 'absent'
    record.cache_write_state = Object.hasOwn(usage, 'cacheWriteTokens') ? 'reported' : 'absent'
    ...
    if (event.data?.message?.source?.kind === 'model') {
      ...
      record.connection_id = connectionIdFromAssistantSource(event.data.message.source) ?? record.connection_id
    }
  }
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (isAttemptSettlement(event, header) || (event.type === 'step/end' && attemptSteps.has(keyOf(data.turn, data.step)))) continue
    switch (event.type) {
      case 'request/header': {
        pendingConfig = data.header?.config ?? data.config ?? undefined
      ...
      case 'assistant/message': {
        ...
        if (record.seq === -1) {
          ...
          if (data.message?.source?.kind === 'model') {
            ...
            record.connection_id = connectionIdFromAssistantSource(data.message.source) ?? null
  ...
  const seedLength = requireOwnershipCut(header)
  for (const record of records.values()) {
    record.owned = record.seq >= 0 && record.seq < seedLength ? 0 : 1
  }
  return records
}
...
export function createLedgerService({
  ...
  // Process-local counter for the effective-request projection (never
  // persisted; a fresh process rebuilds once regardless). Bump
  // classification rule:
  // - `bumpProjectionRevision()` for writes that change the rows the
  //   `dsh_effective_requests` temp table projects: request rows (usage,
  //   failure, outcome; purge/restore/corrections), source title and deleted
  //   state, aliases, projects and project_sources, attribution products and
  //   rules (including the legacy-plan projection), and the Ollama cache
  //   estimate rate (it feeds the table's estimated columns).
  // - `bumpAnalyticsRevision()` alone for writes that only affect read-time
  //   valuation or separate tables: observations, budgets, upstream prices,
  //   price overrides, provider multipliers.
  // Every projection bump also bumps the combined counter, so all
  // revision-keyed caches stay correct; only the expensive rebuild is gated.
  let projectionRevision = 0
  // Process-local counter for saved provider observations. Read-side pools
  // views recompute from the combined revision, but observation-independent
  // views (kpis, series, rankings, ...) key their cache on the combined
  // revision *minus* this counter, so saving an observation invalidates only
  // the views that actually read one.
  let observationRevision = 0
  function bumpAnalyticsRevision() {
    analyticsRevision += 1
    statements.bumpAnalyticsMeta.run(String(analyticsRevision))
    return analyticsRevision
  }
  function bumpObservationRevision() {
    observationRevision += 1
    return bumpAnalyticsRevision()
  }
  function bumpProjectionRevision() {
  ...
  // `bumpAnalyticsRevision` runs only from the returned API (never during
  // this factory body), so referencing `statements` declared below is safe.
  const statements = {
    bumpAnalyticsMeta: db.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('analytics_revision', ?)"),
    ...
    upsertRequest: db.prepare(`
      INSERT INTO requests (session_id, turn, step, attempt, seq, time, provider, model_raw, connection_id, owned,
        input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, cache_read_state, cache_write_state, reasoning_tokens,
        status, estimated, estimator, estimator_version, original_usd_nano, price_version,
        duration_ms, end_reason, failure_type)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(session_id, turn, step, attempt) DO UPDATE SET
        seq = excluded.seq,
        time = excluded.time,
        provider = excluded.provider,
        model_raw = excluded.model_raw,
        connection_id = COALESCE(excluded.connection_id, requests.connection_id),
        owned = excluded.owned,
        input_tokens = excluded.input_tokens,
        output_tokens = excluded.output_tokens,
        cache_read_tokens = excluded.cache_read_tokens,
        cache_write_tokens = excluded.cache_write_tokens,
        cache_read_state = excluded.cache_read_state,
        cache_write_state = excluded.cache_write_state,
        ...
        original_usd_nano = CASE
          WHEN requests.original_usd_nano IS NOT NULL
            ...
            AND requests.cache_read_state = excluded.cache_read_state
            AND requests.cache_write_state = excluded.cache_write_state
    ...
    setRequestOutcome: db.prepare(`
      ...
      WHERE session_id = ? AND turn = ? AND step = ? AND attempt = 0
    `),
    markDeleted: db.prepare('UPDATE sources SET deleted = 1 WHERE session_id = ?'),
    // Hot-path default project identity: fixed 'cwd' shape, prepared once.
    insertCwdProjectIfAbsent: db.prepare(`
      INSERT OR IGNORE INTO projects (id, identity_kind, identity_value, display_name, created_at)
      VALUES (?, 'cwd', ?, ?, ?)
    `),
    insertCwdProjectSourceIfAbsent: db.prepare(`
      INSERT OR IGNORE INTO project_sources (cwd, project_id, source, created_at)
      VALUES (?, ?, 'cwd', ?)
    `),
    // Account-connection writes: read-before-write keeps repeats physically
    // idempotent (no UPDATE, no updated_at churn) for unchanged input.
    readAccountConnection: db.prepare('SELECT provider_id, account_key, display_name, status, auth_kind FROM account_connections WHERE id = ?'),
    upsertAccountConnection: db.prepare(`
    ...
    `),
    readCredentialMetadata: db.prepare('SELECT credential_ref, kind, expires_at FROM credential_metadata WHERE id = ?'),
    upsertCredentialMetadata: db.prepare(`
    ...
    `),
    // Observation ingest: prepared once instead of per saved observation.
    upsertObservedConnection: db.prepare(`
      INSERT INTO account_connections
        (id, provider_id, account_key, display_name, status, auth_kind, created_at, updated_at)
      VALUES (?, ?, NULL, ?, 'connected', 'external', ?, ?)
      ON CONFLICT(id) DO UPDATE SET status = excluded.status, updated_at = excluded.updated_at
    `),
    upsertObservedProduct: db.prepare(`
    ...
    `),
    upsertObservedLimit: db.prepare(`
    ...
    insertObservation: db.prepare(`
      INSERT OR REPLACE INTO account_observations
        (id, connection_id, product_id, observed_at, source_kind, brittle, usable, payload_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `),
    // Per-connection observation retention. Deletes everything outside the
    // newest `OBSERVATION_RETENTION_PER_CONNECTION` rows except the newest
    // usable one: the latest read serves only usable rows, so dropping that
    // row would let a run of window-less probes hide the connection's last
    // real observation forever. When the connection has no usable row the
    // second NOT IN matches nothing and the plain cap applies.
    enforceObservationRetention: db.prepare(`
      DELETE FROM account_observations
      WHERE connection_id = ?
        AND id NOT IN (
          SELECT id FROM account_observations
          WHERE connection_id = ?
        ...
        AND id NOT IN (
          SELECT id FROM account_observations
          WHERE connection_id = ? AND usable = 1
          ORDER BY observed_at DESC, id DESC
          LIMIT 1
        )
    `),
  ...
  /** Persist folded records; returns whether any request row was written. */
  function persistRecords(sessionId, records) {
    let wrote = false
    transaction(db, () => {
      for (const record of records.values()) {
        if (record.seq < 0) continue
        if (record.attempt && db.prepare('SELECT 1 FROM requests WHERE session_id = ? AND turn = ? AND step = ? AND attempt = 0').get(sessionId, record.turn, record.step)) {
          throw new LedgerError('attempt-migration-required', 'legacy step accounting overlaps V4 attempts; explicit ledger reconciliation required')
        }
        const valuation = valueRecord(record)
        statements.upsertRequest.run(
          sessionId, record.turn, record.step, record.attempt ?? 0, record.seq, record.time,
          ...
          record.cache_write_tokens, record.cache_read_state ?? 'unknown', record.cache_write_state ?? 'unknown', record.reasoning_tokens,
  ...
  function ensureDefaultProject(cwd, createdAt = Date.now()) {
    if (!cwd) return null
    const id = `cwd:${cwd}`
    statements.insertCwdProjectIfAbsent.run(id, cwd, basename(cwd) || cwd, createdAt)
    statements.insertCwdProjectSourceIfAbsent.run(cwd, id, createdAt)
    return id
  }
  ...
  function ensureSource(header, source = 'profile') {
    statements.upsertSource.run(
      ...
      inheritedPrefixLength(header) ?? null,
    )
  ...
  return {
    ...
    observationRevision: () => observationRevision,
    // Test/bench seam only: never surfaced through the remote RPC API.
    ...
    updateProject(id, patch = {}) {
      ...
      // Display names label the projection's project rows, so edits must
      // rebuild it; hidden/color are read-time but ride along for free.
      bumpProjectionRevision()
    ...
    correctRequest(requestId, correction = {}) {
      const { sessionId, turn, step, attempt } = parseRequestId(requestId)
      const exists = db.prepare('SELECT 1 FROM requests WHERE session_id = ? AND turn = ? AND step = ? AND attempt = ?').get(sessionId, turn, step, attempt)
      ...
      const result = db.prepare(`
        INSERT INTO request_corrections
          (session_id, turn, step, attempt, input_tokens, output_tokens, cache_read_tokens,
    ...
    revokeCorrection(id) {
      ...
      const latest = db.prepare('SELECT id FROM request_corrections WHERE session_id = ? AND turn = ? AND step = ? AND attempt = ? ORDER BY id DESC LIMIT 1').get(correction.session_id, correction.turn, correction.step, correction.attempt)
      if (Number(latest.id) !== Number(id)) throw new LedgerError('correction-not-current', `correction is no longer current: ${id}`)
      const result = db.prepare(`
        INSERT INTO request_corrections (session_id, turn, step, attempt, excluded, note, active, created_at)
        VALUES (?, ?, ?, ?, 0, ?, 1, ?)
      `).run(correction.session_id, correction.turn, correction.step, correction.attempt, `reverted correction #${id}`, Date.now())
      bumpProjectionRevision()
    ...
    ingestEvent(header, event) {
      try {
        ensureSource(header)
        const data = event.data ?? {}
        // Only writes that change projected rows may bump: metadata-only
        // traffic (headers, step starts, buffered messages, lastSeq) must
        // not pay for a projection rebuild on every event.
        let wroteProjection = false
        if (event.type === 'request/header') {
          liveConfig.set(header.id, data.header?.config ?? data.config ?? undefined)
        ...
        } else if (isAttemptSettlement(event, header)) {
          const start = liveStarts.get(`${header.id}:${data.turn}:${data.step}`)
          const prefix = [
            { type: 'request/header', data: { config: liveConfig.get(header.id) } },
            ...(start === undefined ? [] : [{ type: 'step/start', time: start, data: { turn: data.turn, step: data.step } }]),
          ]
          wroteProjection = persistRecords(header.id, foldEvents(header, [...prefix, event], { estimator }))
        } else if (event.type === 'llm/retry-started') {
          liveStarts.set(`${header.id}:${data.turn}:${data.step}`, event.time)
          ...
          const record = {
            ...
            connection_id: data.message?.source?.kind === 'model' ? connectionIdFromAssistantSource(data.message.source) ?? null : null,
            ...
            cache_read_state: Object.hasOwn(data.usage, 'cacheReadTokens') ? 'reported' : 'absent',
            cache_write_state: Object.hasOwn(data.usage, 'cacheWriteTokens') ? 'reported' : 'absent',
            reasoning_tokens: data.usage.reasoningTokens ?? null,
            status: 'ok', estimated: 0, estimator: null, estimator_version: null,
            owned: event.seq < (inheritedPrefixLength(header) ?? 0) ? 0 : 1,
          }
          wroteProjection = persistRecords(header.id, new Map([[`${turn}:${step}`, record]]))
          ...
          if (failed) {
            ...
            const inserted = statements.insertFailureIfAbsent.run(
              header.id, turn, step, event.seq, event.time,
              config?.provider ?? 'unknown', config?.model ?? 'unknown',
              event.seq < (inheritedPrefixLength(header) ?? 0) ? 0 : 1,
              header.id, turn, step,
            )
            wroteProjection = Number(inserted.changes) > 0
          }
          ...
          const reason = data.reason
          const failure = data.error
          const outcome = statements.setRequestOutcome.run(
          ...
          if (Number(outcome.changes) > 0) wroteProjection = true
        ...
        if (wroteProjection) bumpProjectionRevision()
      } catch (error) {
    ...
    reconcileSources(known) {
      ...
      if (deleted.length) bumpProjectionRevision()
      return { deleted }
    },
    ...
    getOverview(options = {}) {
      ...
      for (const row of rows) {
        ...
        if (row.status === 'failed' || (row.attempt > 0 && row.failure_type !== null)) {
    ...
    getDailySeries({ from, to, timezone = defaultTimezone } = {}) {
      ...
      const countStmt = db.prepare(`
        SELECT COUNT(*) AS requests,
               ...
               SUM(CASE WHEN status = 'failed' OR (attempt > 0 AND failure_type IS NOT NULL) THEN 1 ELSE 0 END) AS failed,
    ...
    getSessionDetail(sessionId, options = {}) {
      ...
      for (const row of rows) {
        ...
        if (row.status === 'failed' || (row.attempt > 0 && row.failure_type !== null)) own.failedRequests += 1
        own.requests += 1
      ...
      return {
        ...
        includingChildren,
        calls: rows.map((row) => ({
          turn: row.turn, step: row.step, attempt: row.attempt ?? 0, time: row.time,
    ...
    listRequests({ sessionId, model, provider, status, estimated, fromMs, toMs, limit = 50, offset = 0 } = {}) {
      ...
      return {
        rows: rows.map((row) => {
          ...
          return {
            sessionId: row.session_id, turn: row.turn, step: row.step, attempt: row.attempt ?? 0, time: row.time,
    ...
    setAlias(modelRaw, canonical) {
      ...
      // Aliases are joined inside the effective projection; a change must
      // rebuild it, not just invalidate result caches.
      bumpProjectionRevision()
    },
    ...
    saveAccountConnection(connection) {
      ...
      const status = connection.configured === true ? 'connected' : 'not_connected'
      const authKind = connection.credentialKind ?? 'none'
      const accountKey = connection.accountKey ?? null
      const displayName = connection.displayName ?? connection.providerId
      // Physical idempotency: identical input performs no write at all, so
      // startup-time re-saves never churn `updated_at` or dirty caches.
      const existing = statements.readAccountConnection.get(id)
      const current = existing === undefined ? null : {
        providerId: existing.provider_id,
        accountKey: existing.account_key ?? null,
        displayName: existing.display_name,
        status: existing.status,
        authKind: existing.auth_kind,
      }
      const unchanged = current !== null
        ...
        && current.authKind === authKind
      let changed = !unchanged
      if (changed) {
        statements.upsertAccountConnection.run(id, connection.providerId, accountKey, displayName, status, authKind, now, now)
      }
      if (typeof connection.credentialRef === 'string' && connection.credentialRef) {
        const kind = connection.credentialKind ?? 'unknown'
        const expiresAt = connection.credentialExpiresAt ?? null
        const credential = statements.readCredentialMetadata.get(`${id}:credential`)
        const credentialUnchanged = credential !== undefined
          && credential.credential_ref === connection.credentialRef
          && credential.kind === kind
          && (credential.expires_at ?? null) === expiresAt
        if (!credentialUnchanged) {
          statements.upsertCredentialMetadata.run(`${id}:credential`, id, connection.credentialRef, kind, expiresAt, now)
          changed = true
        }
      }
      return { id, changed }
    ...
    saveAccountObservation(observation) {
      ...
      transaction(db, () => {
        ...
        statements.upsertObservedConnection.run(connectionId, providerId, providerId, observation.observedAt, observation.observedAt)
        if (observation.product) {
          statements.upsertObservedProduct.run(observation.product.id, observation.providerId, observation.product.externalId ?? observation.product.code ?? null,
        ...
        for (const limit of observation.limits ?? []) {
          ...
          statements.upsertObservedLimit.run(limit.id, connectionId, limit.productId ?? observation.product?.id ?? null,
        ...
        statements.insertObservation.run(observation.id, connectionId, observation.product?.id ?? null, observation.observedAt,
          observation.source, observation.brittle ? 1 : 0, observationIsUsable(observation) ? 1 : 0, serialized)
        // Bound the per-connection history in the same transaction as the
        // insert, so a crash can never leave the cap exceeded and the newest
        // usable row is retired together with everything else.
        statements.enforceObservationRetention.run(connectionId, connectionId, OBSERVATION_RETENTION_PER_CONNECTION, connectionId)
        // Official windows feed pool summaries and account details, which are
        // revision-keyed caches: bumping the combined revision makes the fresh
        // observation visible on the next read immediately. Observations are
        // read-side facts, so this stays clear of the expensive projection
        // rebuild, which only projection revisions may trigger — and the
        // observation component of the revision lets observation-independent
        // views keep their cached results.
        bumpObservationRevision()
      })
      return observation
    },

    /**
     * Stamp a provider's connection onto an existing unarchived account that
     * has no connection link (template/manual origin). Observations are
     * connection-keyed, so this link is what lets official windows reach an
     * account the user created before the connection existed. Idempotent:
     * an already-linked provider is a no-op.
     */
    linkAccountConnection({ providerId, connectionId } = {}) {
      if (typeof providerId !== 'string' || providerId.length === 0) throw new LedgerError('bad-connection', 'linkAccountConnection requires providerId')
      if (typeof connectionId !== 'string' || connectionId.length === 0) throw new LedgerError('bad-connection', 'linkAccountConnection requires connectionId')
      const candidate = db.prepare(`
        SELECT id, connection_id AS connectionId FROM account_products
        WHERE provider_id = ? AND archived_at IS NULL AND connection_id IS NULL
        ORDER BY created_at, id LIMIT 1
      `).get(providerId)
      if (candidate === undefined) return { id: null, linked: false }
      db.prepare('UPDATE account_products SET connection_id = ? WHERE id = ?').run(connectionId, candidate.id)
      // The claimed link rewrites attribution, which the projection joins on.
      bumpProjectionRevision()
      return { id: candidate.id, linked: true }
    },
    ...
    ensureConnectionAccount(connection, { aliases = [], attribution = 'provider' } = {}) {
      ...
      // Products and connection-default rules feed the projection's pool
      // join: rebuild only when something actually changed.
      if (changed) bumpProjectionRevision()
    ...
    listAccounts() {
      ...
      const limits = db.prepare(`
        SELECT id, product_id AS productId, external_key AS externalKey, metric, unit,
               ...
               window_seconds AS windowSeconds, window_json AS windowJson, source_kind AS sourceKind
      ...
      for (const limit of limits) {
        ...
        const anchor = windowAnchor(windowJson)
        const rows = limitsByProduct.get(limit.productId) ?? []
        // The anchor is optional structure: it appears only on limits that
        // declare one, so plain rows keep their historical shape.
        rows.push({ ...cleanLimit, exactValue: limit.exactValue == null ? null : Number(limit.exactValue), ...(anchor !== null ? { anchor } : {}) })
    ...
    saveAccount(account = {}) {
      ...
      const cleanLimits = limits.map((limit, index) => {
        ...
        // Calendar anchors for fixed windows ride the existing window_json
        // column — no schema change; invalid members are dropped silently.
        const anchor = normalizeAnchor(limit?.anchor ?? limit?.window?.anchor)
        return { externalKey, unit, valueMode, value, windowKind, windowSeconds, anchor }
      })
      ...
      transaction(db, () => {
        ...
        const insertLimit = db.prepare(`
          INSERT INTO account_limits (id, product_id, external_key, metric, unit, value_mode, exact_value,
            window_kind, window_seconds, window_json, source_kind, confidence, note, created_at)
          VALUES (?, ?, ?, 'quota', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `)
        for (const limit of cleanLimits) {
          insertLimit.run(`account-limit:${id}:${limit.externalKey}`, id, limit.externalKey, limit.unit, limit.valueMode,
            limit.value == null ? null : String(limit.value), limit.windowKind, limit.windowSeconds,
            limit.anchor == null ? null : JSON.stringify({ anchor: limit.anchor }), sourceKind,
      ...
      // Products, quota limits and attribution rules all feed the projection.
      bumpProjectionRevision()
    ...
    purgeBefore(cutoffMs, { timezone: tz = DEFAULT_TIMEZONE } = {}) {
      ...
      if (result.deleted > 0) bumpProjectionRevision()
      return result
    },
    ...
    setOllamaCacheEstimateBps(bps) {
      // The rate feeds the projection's estimated columns; analytics invalidates
      // its own projection handle, the bump keeps both counters in lockstep.
      const value = analytics.setOllamaCacheEstimateBps(bps)
      bumpProjectionRevision()
      return value
    },
    ...
    restoreFrom(backupPath, { mode = 'merge' } = {}) {
      ...
      try {
        transaction(db, () => {
          ...
          for (const table of tables) {
            ...
            const sourceColumns = new Set(db.prepare(`PRAGMA restore_src.table_info('${table}')`).all().map((column) => column.name))
            const common = targetColumns.filter((column) => sourceColumns.has(column))
            if (common.length === 0) continue
            const columns = common.map((column) => `"${String(column).replaceAll('"', '""')}"`).join(', ')
            const conflict = mode === 'replace' ? '' : ' OR IGNORE'
            db.exec(`INSERT${conflict} INTO main.${table} (${columns}) SELECT ${columns} FROM restore_src.${table}`)
          }
        })
      } finally {
      ...
      // Restores rewrite request rows, sources, projects, aliases and rules.
      bumpProjectionRevision()
    },
