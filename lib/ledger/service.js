...
export function createLedgerService({
  ...
  return {
    ...
    listPlans() {
      ...
      `).all().map((row) => ({ ...row, quotaValue: row.quotaValue == null ? null : Number(row.quotaValue), archived: row.archivedAt != null }))
    ...
    savePlan(plan = {}) {
      ...
      db.prepare(`
        INSERT INTO plans (id, kind, name, color, price_usd_nano, quota_unit, quota_value, reset_day, balance_usd_nano, expiry_day, archived_at, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?)
        ON CONFLICT(id) DO UPDATE SET kind = excluded.kind, name = excluded.name, color = excluded.color,
          ...
          archived_at = NULL
      `).run(id, kind, name, plan.color ?? null, priceUsdNano, quotaUnit, quotaValue, resetDay, balanceUsdNano, expiryDay, Date.now())
    ...
    savePlanRules(planId, rules = []) {
      ...
      bumpAnalyticsRevision()
      return { planId: String(planId), count: clean.length }
    },
