...
export function createLedgerService({
  ...
  const statements = {
    ...
    allSources: db.prepare('SELECT session_id, revision FROM sources WHERE deleted = 0'),
  }
  ...
  return {
        ...
        if (event.type === 'request/header') {
          ...
          if (failed) {
            ...
            const record = {
              ...
              model_raw: config?.model ?? 'unknown',
              input_tokens: 0, output_tokens: 0, cache_read_tokens: 0, cache_write_tokens: 0,
              reasoning_tokens: null, status: 'failed', estimated: 0,
              estimator: null, estimator_version: null,
              owned: header.seedLength !== undefined && header.seedLength !== null && event.seq < header.seedLength ? 0 : 1,
            }
            persistRecords(header.id, new Map([[`${turn}:${step}`, record]]))
          }
    ...
    /** Mark sources absent from `known` as deleted; history is retained. */
    reconcileSources(known) {
      const knownIds = new Set(known.map((entry) => entry.id))
      const deleted = []
      for (const row of statements.allSources.all()) {
    ...
    getSessionDetail(sessionId, options = {}) {
      ...
      for (const child of children) {
        for (const row of db.prepare('SELECT * FROM requests WHERE session_id = ?').all(child.id)) {
    ...
    /**
     * Restore a backup. `replace` wipes and copies the backup verbatim;
     * `merge` inserts only rows whose keys are absent (idempotent, never
     * overwrites local facts).
     */
