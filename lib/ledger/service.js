...
export function createLedgerService({
  ...
  return {
    ...
    saveAccountObservation(observation) {
      ...
      transaction(db, () => {
        const connectionId = observation.connectionId ?? `${observation.providerId}:default`
        db.prepare(`INSERT INTO account_connections
          (id, provider_id, account_key, display_name, status, auth_kind, created_at, updated_at)
          VALUES (?, ?, NULL, ?, 'connected', 'external', ?, ?)
          ON CONFLICT(id) DO UPDATE SET status = excluded.status, updated_at = excluded.updated_at
        `).run(connectionId, observation.providerId, observation.providerId, observation.observedAt, observation.observedAt)
     ...
     */
    ensureConnectionAccount(connection, { aliases = [], attributable = true } = {}) {
      if (!connection || typeof connection.providerId !== 'string') throw new LedgerError('bad-connection', 'account connection requires providerId')
      ...
      if (attributable) {
        // Connection-default rules are system-owned: rewrite them only when
        // the desired alias set differs, which also removes any duplicates
        // produced by earlier versions. User-authored rules are untouched.
        const desired = [...new Set((aliases.length > 0 ? aliases : [connection.providerId]).map((alias) => `${alias}*`))].sort()
        const existing = db.prepare(`
          SELECT match_provider AS matchProvider FROM account_attribution_rules
          WHERE product_id = ? AND source_kind = 'connection_default' ORDER BY match_provider
        `).all(productId).map((rule) => rule.matchProvider).sort()
    ...
    listAccounts() {
      ...
      const rules = db.prepare(`
        SELECT id, product_id AS productId, match_provider AS matchProvider,
