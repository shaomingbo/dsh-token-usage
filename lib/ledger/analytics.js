...
import { requestId, parseRequestId } from './attempts.js'
...
function aggregateColumns(includeEstimates) {
  ...
  return `
    ...
    COALESCE(SUM(CASE WHEN (r.status = 'failed' OR (r.attempt > 0 AND r.failure_type IS NOT NULL)) THEN 1 ELSE 0 END), 0) AS failedRequests,
...
export function createAnalytics({
  ...
  const effectiveSelect = `
    ...
    SELECT r.session_id, r.turn, r.step, r.attempt, r.seq, r.time, r.provider, r.model_raw, r.connection_id,
    ...
    LEFT JOIN main.request_corrections c ON c.id = (
      ...
      WHERE rc.session_id = r.session_id AND rc.turn = r.turn AND rc.step = r.step AND rc.attempt = r.attempt AND rc.active = 1
  ...
  function entrySummary({ now = Date.now(), timezone = defaultTimezone } = {}) {
    ...
    return {
      ...
      tightest: tightest === null ? null : {
        ...
        providerId: tightest.providerId ?? null,
        connectionId: tightest.connectionId ?? null,
      ...
      pools: pools.pools.map((pool) => {
        ...
        return {
          ...
          color: pool.color,
          kind: pool.kind,
          providerId: pool.providerId ?? null,
          connectionId: pool.connectionId ?? null,
          usedPct: pool.usedPct == null ? null : Math.round(pool.usedPct * 10) / 10,
          // The official windows ride along so the sidebar footer can meter
          // the same watched window the dock row stars (client-side pick).
    ...
    if (views.has('page')) {
      ...
      if (entity === 'session') {
        ...
        if (cursor) {
          ...
          if (!Number.isSafeInteger(cursor.attempt ?? 0)) throw new TypeError('invalid attempt cursor')
          pageCompiled.clauses.push(`(r.time < ? OR (r.time = ? AND (r.session_id, r.turn, r.step, r.attempt) > (?, ?, ?, ?)))`)
          pageCompiled.params.push(cursor.time, cursor.time, cursor.id, cursor.turn, cursor.step, cursor.attempt ?? 0)
        }
        const rows = db.prepare(`${PROJECT_CTE}
          SELECT r.*, r.project, r.project_label AS project_label
          FROM dsh_effective_requests r
          WHERE ${pageCompiled.clauses.join(' AND ')}
          ORDER BY r.time DESC, r.session_id, r.turn, r.step, r.attempt LIMIT ?
        `).all(...pageCompiled.params, limit + 1)
        result.page = {
          entity,
          rows: rows.slice(0, limit).map((row) => requestRow(row, catalog)),
          nextCursor: rows.length > limit ? Buffer.from(JSON.stringify({ revision: revision(), time: rows[limit - 1].time, id: rows[limit - 1].session_id, turn: rows[limit - 1].turn, step: rows[limit - 1].step, attempt: rows[limit - 1].attempt })).toString('base64url') : null,
        }
      }
    }
  ...
  function inspect(ref = {}) {
    ...
    if (ref.kind === 'request') {
      let identity
      try { identity = parseRequestId(ref.id) } catch { return null }
      const { sessionId, turn, step, attempt } = identity
      const row = db.prepare(`${PROJECT_CTE}
        SELECT r.*, r.project, r.project_label AS project_label FROM dsh_effective_requests r
        WHERE r.session_id = ? AND r.turn = ? AND r.step = ? AND r.attempt = ?
      `).get(sessionId, turn, step, attempt)
      if (row === undefined) return null
      const item = requestRow(row, catalog)
      return {
        kind: 'request', id: String(ref.id), filter,
        asOf: { revision: revision(), generatedAtMs: now, timezone },
        identity: { sessionId, turn, step, attempt, project: item.project, model: item.model, provider: item.provider, status: item.status },
        direct: {
          requests: 1,
          calls: item.status === 'ok' ? 1 : 0,
          failedRequests: item.status === 'failed' || (item.attempt > 0 && item.failureType !== null) ? 1 : 0,
        ...
        corrections: db.prepare(`
          ...
          FROM request_corrections WHERE session_id = ? AND turn = ? AND step = ? AND attempt = ? ORDER BY id DESC
        `).all(sessionId, turn, step, attempt).map((correction) => ({ ...correction, excluded: correction.excluded === 1, active: correction.active === 1 })),
      }
    }
