...
/** Resolve a declared quota limit into a measurable analytics window. */
function quotaWindow(limit, resetDay, now, timezone) {
  ...
  if (limit.windowKind === 'billing') {
    ...
    return { key: `billing:${resetDay ?? 1}`, ...cycle }
  }
  const seconds = Number(limit.windowSeconds)
  if ((limit.windowKind === 'rolling' || limit.windowKind === 'fixed') && Number.isFinite(seconds) && seconds > 0) {
    return {
      key: `roll:${seconds}`,
...
export function createAnalytics({ db, catalog, timezone: defaultTimezone = 'UTC', revision = () => 0 } = {}) {
  ...
  function ensureEffectiveRequests(currentRevision = revision()) {
    ...
    db.exec('CREATE TEMP TABLE dsh_pool_rules (provider TEXT NOT NULL, model TEXT NOT NULL, product_id TEXT NOT NULL)')
    // Account attribution rules are the single attribution source: template
    // and connection-default rules live here, and the v5 lossless projection
    // keeps legacy plan_rules working through the same table.
    const rules = db.prepare(`
      SELECT ar.match_provider AS matchProvider, ar.match_model AS matchModel, ar.product_id AS productId
    ...
    if (rules.length > 0) {
      ...
      const insert = db.prepare('INSERT INTO dsh_pool_rules (provider, model, product_id) VALUES (?, ?, ?)')
      transaction(db, () => {
        for (const pair of pairs) {
          ...
          for (const rule of rules) {
            ...
            if (providerOk && modelOk) { insert.run(pair.provider ?? '', model ?? '', rule.productId); break }
          }
        }
      })
    }
  ...
  function listAccountProducts() {
    ...
    const limits = db.prepare(`
      SELECT product_id AS productId, external_key AS externalKey, metric, unit,
             ...
             reset_at AS resetAt, source_kind AS sourceKind, confidence, created_at AS createdAt
      FROM account_limits
      WHERE metric = 'quota'
   ...
   * percentages into token guesses.
   */
  function latestObservationsByConnection() {
    const rows = db.prepare(`
      SELECT connection_id AS connectionId, payload_json AS payload
      FROM account_observations ao
      WHERE connection_id IS NOT NULL
        AND observed_at = (SELECT MAX(o2.observed_at) FROM account_observations o2 WHERE o2.connection_id = ao.connection_id)
    `).all()
    ...
    for (const row of rows) {
      try {
        ...
        byConnection.set(String(row.connectionId), {
  ...
  function computePoolsUncached(now, timezone, includeEstimates) {
    ...
    if (products.length === 0) {
      return { configured: false, month, pools: [], unassigned, tightestPoolId: null }
    }
    scan('7d', last7)
    ...
    const pools = products.map((product) => {
      const official = product.connectionId != null ? observations.get(product.connectionId) ?? null : null
      const quotaWindows = []
      ...
      const officialPct = official === null ? null : official.windows.reduce((best, window) => Math.max(best ?? 0, window.percentUsed), null)
  ...
  /** Sidebar-entry micro summary: tightest account + month progress, minimal payload. */
  function entrySummary({ now = Date.now(), timezone = defaultTimezone } = {}) {
    ...
    let resetsAt = null
    let sourceKind = 'local_ledger'
    if (tightest !== null) {
      const officialWindow = tightest.official == null ? null
        : tightest.official.windows.reduce((best, window) => (best === null || window.percentUsed > best.percentUsed ? window : best), null)
      const localKey = tightest.quotaWindows.find((entry) => entry.usedPct != null && entry.usedPct === tightest.localUsedPct)?.externalKey ?? null
      if (officialWindow !== null && (tightest.localUsedPct == null || officialWindow.percentUsed >= tightest.localUsedPct)) {
        windowLabel = officialWindow.label ?? officialWindow.externalKey ?? null
        resetsAt = officialWindow.resetsAt ?? null
        sourceKind = tightest.official.sourceKind
      } else {
        windowLabel = localKey
      }
    }
    return {
