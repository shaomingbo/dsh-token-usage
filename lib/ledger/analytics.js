...
function quotaWindow(kind, resetDay, now, timezone) {
  if (kind === '5h') return { key: '5h', fromMs: now - 5 * HOUR, toMs: now, daysLeft: 5 / 24, elapsedPct: null, resetLabel: null }
  if (kind === '7d') return { key: '7d', fromMs: now - 7 * DAY, toMs: now, daysLeft: 7, elapsedPct: null, resetLabel: null }
  if (kind === 'month') {
    const cycle = billingCycle(now, resetDay ?? 1, timezone)
    return { key: `month:${resetDay ?? 1}`, ...cycle }
  }
  return null
}
...
export function createAnalytics({ db, catalog, timezone: defaultTimezone = 'UTC', revision = () => 0 } = {}) {
  ...
  function listPlanRows() {
    ...
    `).all().map((row) => ({
      ...
      windowKind: row.windowKind ?? 'month',
    }))
  }
  ...
  function computePoolsUncached(now, timezone, includeEstimates) {
    ...
    const last5h = { fromMs: now - 5 * HOUR, toMs: now }
    const scans = new Map()
    ...
    scan('30d', last30)
    scan('7d', last7)
    if (plans.some((plan) => plan.windowKind === '5h' || plan.window2Kind === '5h')) scan('5h', last5h)
    for (const plan of plans) {
      if (plan.kind === 'sub' && (plan.windowKind === 'month' || plan.windowKind == null)) {
        const cycle = billingCycle(now, plan.resetDay, timezone)
        scan(`month:${plan.resetDay ?? 1}`, cycle)
      }
    }
    const pack = (scanKey, planId) => {
      const bucket = scans.get(scanKey)
      if (!bucket) return { ...measures({}), cost: measures({}).cost }
      return { ...(bucket.measures.get(planId) ?? measures({})), cost: { ...measures({}).cost, ...(bucket.costs.get(planId) ?? {}) } }
    }
    const pools = plans.map((plan) => {
      if (plan.kind === 'credit') {
        const kpis = pack('30d', plan.id)
      ...
      const primary = quotaWindow(plan.windowKind ?? 'month', plan.resetDay, now, timezone)
      const kpis = primary ? pack(primary.key, plan.id) : pack('30d', plan.id)
      const last7Kpis = pack('7d', plan.id)
      const used = usedAmount(kpis, plan.quotaUnit)
      const rate = usedAmount(last7Kpis, plan.quotaUnit) / 7
      const quota = plan.quotaValue ?? 0
      const usedPct = primary && quota > 0 ? (used / quota) * 100 : null
      const remaining = primary && quota > 0 ? Math.max(0, quota - used) : null
      const daysToCap = remaining !== null && rate > 0 && primary.daysLeft != null && remaining / rate < primary.daysLeft ? remaining / rate : null
      const leftoverAtReset = plan.windowKind === 'month' && quota > 0 ? Math.max(0, quota - (used + rate * (primary.daysLeft ?? 0))) : null
      const secondary = quotaWindow(plan.window2Kind, plan.resetDay, now, timezone)
      const secondaryKpis = secondary ? pack(secondary.key, plan.id) : null
      const secondaryUsed = secondaryKpis ? usedAmount(secondaryKpis, plan.window2QuotaUnit ?? plan.quotaUnit) : null
      const secondaryUsedPct = secondary && plan.window2QuotaValue > 0 ? (secondaryUsed / plan.window2QuotaValue) * 100 : null
      return {
        ...plan,
        cycle: primary ?? last30,
        kpis,
        used,
        ratePerDay: rate,
        usedPct,
        ...
        secondaryUsedPct,
        secondaryWindowKind: plan.window2Kind ?? null,
      }
    })
