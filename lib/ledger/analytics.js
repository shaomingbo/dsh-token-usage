...
export function createAnalytics({
  ...
  /** Rebuild counter for tests and benchmarks; never part of the wire API. */
  function diagnostics() {
    return { effectiveRebuilds }
  }
  ...
  function latestObservationsByConnection() {
    // Newest-first, skip payloads with no percent windows so an API-key
    // reachability probe cannot hide a previous settings-page scrape.
  ...
  function computePoolsUncached(now, timezone, includeEstimates) {
    ...
    const pools = products.map((product) => {
      const official = observationForConnection(observations, product.connectionId)
      const quotaWindows = []
  ...
  function query(spec = {}) {
    ...
    const { now: _ignoredNow, ...keySpec } = spec
    const cacheKey = JSON.stringify({ revision: revisionValue, clock: Math.floor(now / CLOCK_BUCKET_MS), ...keySpec, filter })
    ...
    if (views.has('seriesBy')) {
      ...
      // cache dimension-free series by revision, a coarse clock, and the
      // *description* of filter.time (preset or explicit bounds) — never the
      // resolved window, whose rolling `toMs` would drift with every poll.
      // A filtered exploration recomputes (it is inherently narrower).
      const seriesCacheKey = dimensionFiltered ? null
        : JSON.stringify({ revision: revisionValue, clock: Math.floor(now / CLOCK_BUCKET_MS), timezone, groupBy, time: filter.time ?? null, includeEstimates })
