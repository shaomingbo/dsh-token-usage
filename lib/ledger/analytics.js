...
export function createAnalytics({ db, catalog, timezone: defaultTimezone = 'UTC', revision = () => 0 } = {}) {
  ...
  function poolWindow(pool) {
    ...
    const officialWindow = pool.official == null ? null
      : (pool.official.windows ?? []).reduce((best, window) => (best === null || window.percentUsed > best.percentUsed ? window : best), null)
    if (officialWindow !== null && (pool.localUsedPct == null || officialWindow.percentUsed >= pool.localUsedPct)) {
      return {
