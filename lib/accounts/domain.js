...
/**
 * Calendar anchor for fixed windows: `{ weekday?: 1..7 (ISO, 1 = Monday),
 * hour?: 0..23, timezone?: IANA }`. Anchors are advisory structure — an
 * invalid member is dropped rather than failing the whole record, and an
 * anchor with no valid member normalizes to null.
 */
export function normalizeAnchor(value) {
  if (value === undefined || value === null || !isPlainObject(value)) return null
  const result = {}
  if (Number.isInteger(value.weekday) && value.weekday >= 1 && value.weekday <= 7) result.weekday = value.weekday
  if (Number.isInteger(value.hour) && value.hour >= 0 && value.hour <= 23) result.hour = value.hour
  if (validTimezone(value.timezone)) result.timezone = value.timezone.trim()
...
export function normalizeLimit(input) {
  const result = {
    ...
    anchor: normalizeAnchor(input.anchor ?? input.window?.anchor),
