...
export class OllamaCloudModels {
  constructor({ fetch, settings, allowedOrigins = OLLAMA_OFFICIAL_ORIGINS } = {}) {
    ...
    if (!settings || typeof settings.read !== 'function' || typeof settings.mutate !== 'function') {
  ...
  status() {
    const configured = this.settings.read(OLLAMA_CLOUD_ROUTE_ID)?.value
    return {
  ...
  async sync({ apiKey, signal } = {}) {
    ...
    const snapshot = this.settings.read(OLLAMA_CLOUD_ROUTE_ID)
    if (!snapshot) throw new AccountsError('settings-unavailable', 'Ollama Cloud provider settings unavailable')
    const currentRoute = snapshot.value
    ...
    const { models: _models, ...routeFields } = route(undefined, models)
    const ops = this.settings.fields(routeFields)
    const oldModels = currentRoute?.models
    if (!Array.isArray(oldModels) || oldModels.length === 0) {
      ops.push({ op: 'set', path: ['models'], value: models })
    } else {
      // Retain existing order/objects (including redacted secrets). Remove only
      // explicitly retired model IDs, then patch public leaves and append new IDs.
      const desired = new Map(models.map(model => [model.id, model]))
      for (let index = oldModels.length - 1; index >= 0; index--) {
        if (!desired.has(oldModels[index].id)) ops.push({ op: 'unset', path: ['models', String(index)] })
      }
      const retained = oldModels.filter(model => desired.has(model.id))
      retained.forEach((model, index) => {
        ops.push(...this.settings.fields(desired.get(model.id), ['models', String(index)]))
        desired.delete(model.id)
      })
      let index = retained.length
      for (const model of desired.values()) ops.push({ op: 'set', path: ['models', String(index++)], value: model })
    }
    await this.settings.mutate(snapshot, ops)
    return {
      providerId: OLLAMA_CLOUD_ROUTE_ID,
      routeId: OLLAMA_CLOUD_ROUTE_ID,
