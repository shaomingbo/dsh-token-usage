...
export class GlmAdapter {
  ...
  async observe({ connection = {}, credential = {}, now = Date.now, signal } = {}) {
    ...
    const parsed = entries.map((entry, index) => parseLimit(entry, index, connection.id, observedAt)).filter(Boolean)
      .sort((left, right) => glmLimitRank(left) - glmLimitRank(right))
    const warnings = []
...
function glmLimitType(entry) {
  return String(entry?.type ?? entry?.metric ?? entry?.limitType ?? '').toUpperCase()
}

function glmLimitRank(parsed) {
  const type = String(parsed?.limit?.metadata?.providerType ?? '').toUpperCase()
  if (type === 'TOKENS_LIMIT') return 0
  if (type === 'TIME_LIMIT') return 1
  return 2
}

function officialLimitLabel(entry, index) {
  const type = glmLimitType(entry)
  if (type === 'TOKENS_LIMIT') return '5小时'
  if (type === 'TIME_LIMIT') return 'MCP'
  const name = typeof entry.name === 'string' ? entry.name.trim() : ''
  if (name && !/^(TIME|TOKENS)_LIMIT$/i.test(name)) return name
  return name || type || `quota-${index + 1}`
}
...
function parseLimit(entry, index, connectionId, observedAt) {
  ...
  const label = officialLimitLabel(entry, index)
