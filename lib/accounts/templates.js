...
export function validateTemplateCatalog(catalog) {
  ...
  catalog.templates.forEach((template, index) => {
    ...
    limits.forEach((limit, limitIndex) => {
      ...
      // Calendar anchors ride on fixed windows and are stored verbatim with
      // the limit row (seeding serializes limits as-is), so validation is the
      // only pass that can stop a malformed anchor.
      if (limit.anchor !== undefined && limit.anchor !== null) {
        if (!isPlainObject(limit.anchor)) fail(`${limitAt}.anchor must be an object`)
        const { weekday, hour, timezone } = limit.anchor
        if (weekday != null && (!Number.isInteger(weekday) || weekday < 1 || weekday > 7)) {
          fail(`${limitAt}.anchor.weekday must be an ISO weekday from 1 (Monday) to 7`)
        }
        if (hour != null && (!Number.isInteger(hour) || hour < 0 || hour > 23)) {
          fail(`${limitAt}.anchor.hour must be an hour from 0 to 23`)
        }
        if (timezone != null) {
          if (typeof timezone !== 'string' || !timezone.trim()) fail(`${limitAt}.anchor.timezone must be a non-empty IANA name`)
          try {
            new Intl.DateTimeFormat('en-US', { timeZone: timezone.trim() })
          } catch {
            fail(`${limitAt}.anchor.timezone must be a known IANA time zone`)
          }
        }
      }
    })
