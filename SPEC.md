...
91. As a Harness user, I want the wide sidebar entry to switch among pools, badge, plain, and hidden display modes, so that it stays glanceable without opening the dashboard.
...
- Pricing precedence is explicit user override (raw name or alias target), the alias-target catalog price, then refreshed upstream prices, and finally the bundled snapshot. A catalog the user explicitly refreshes and confirms is intended to outrank the bundled snapshot because it is newer; refreshed prices win across exact, normalized, provider-prefixed, and alias-target matches. Unknown prices contribute tokens and requests but not cost; aggregates expose cost coverage.
...
- The wide sidebar switches among pools (level bars), badge, plain (entry title only), and hidden modes according to a local setting. The narrow sidebar shows only the usage icon.
