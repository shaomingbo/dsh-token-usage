...
- **四空间分析工作台** —— 总览、探索、成本与预算、数据与设置共享同一全局范围。点击趋势桶和排行维度只会继续收窄；项目/模型/提供方 → 会话 → 请求在详情栈中下钻而不丢上下文。跟随 Harness 主题，支持键盘、响应式布局和中英文。
- **统一分析** —— 默认近 30 天、仅已上报、自动对比等长上期；提供处理 Token、新计算、当前规则费用、请求数，带有界堆叠趋势、构成、维度排行、12 个月活动、星期×小时节奏、中性会话标签，以及最多三条有证据的确定性洞察。
- **本地控制** —— Git/手工项目身份支持合并、拆分、重命名、配色和隐藏；请求修正/排除采用追加式审计且可撤销；自然月预算覆盖 profile/项目/提供方/模型与 USD/处理/新计算单位，并使用保守的预测门槛。
...
- 告警自动化、账单/额度对账、agent/skill/tool 归因、跨 profile 聚合、云同步和自动汇率仍不在范围内。见 [V2-PLAN.md](V2-PLAN.md)。
